...
export function CoachDashboardPage({ lang, copy, onLogout, onToggleLang }: CoachDashboardPageProps) {
    ...
    <div className="flex h-screen bg-[#0f0f12] text-white overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-white/10 flex items-center justify-between px-8 bg-[#0f0f12]/95 backdrop-blur z-20">
          ...
          <div className="flex items-center gap-4"><div className="text-right hidden md:block"><div className="text-xs text-gray-400">{copy.header.value}</div><div className="text-lg font-bold font-mono text-green-400">¥ 458,290</div></div><Avatar className="border border-white/20"><AvatarImage src="https://github.com/shadcn.png" /><AvatarFallback>CX</AvatarFallback></Avatar></div>
        </header>
        ...
        <div className="flex-1 overflow-hidden flex">
          <div className="flex-1 overflow-y-auto p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <Card className="bg-[#141418] border-white/5"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-blue-500/10 rounded-lg text-blue-400"><Disc className="w-6 h-6" /></div><div><div className="text-2xl font-bold">142</div><div className="text-xs text-gray-500">{copy.monitor.kpi_songs}</div></div></CardContent></Card>
              <Card className="bg-[#141418] border-white/5"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-green-500/10 rounded-lg text-green-400"><TrendingUp className="w-6 h-6" /></div><div><div className="text-2xl font-bold">85%</div><div className="text-xs text-gray-500">{copy.monitor.kpi_rate}</div></div></CardContent></Card>
              <Card className="bg-[#141418] border-white/5"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-red-500/10 rounded-lg text-red-400"><MessageSquare className="w-6 h-6" /></div><div><div className="text-2xl font-bold">12</div><div className="text-xs text-gray-500">{copy.monitor.kpi_review}</div></div></CardContent></Card>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {TRAINEES.map((trainee) => (
                    <tr key={trainee.id} onClick={() => setSelectedTrainee(trainee.id)} className={`hover:bg-white/5 cursor-pointer transition-colors ${selectedTrainee === trainee.id ? 'bg-purple-900/20' : ''}`}>
                      <td className="p-4 flex items-center gap-3"><Avatar className="w-8 h-8 rounded-md"><AvatarImage src={trainee.avatar} /><AvatarFallback>{trainee.name[0]}</AvatarFallback></Avatar><div><div className="font-bold">{trainee.name}</div><div className="text-xs text-gray-500">{trainee.lastActive}</div></div></td>
                      <td className="p-4"><Badge variant="outline" className={`${trainee.status === 'On Track' ? 'border-green-500/30 text-green-400 bg-green-500/10' : ''} ${trainee.status === 'Warning' ? 'border-red-500/30 text-red-400 bg-red-500/10' : ''} ${trainee.status === 'Star' ? 'border-yellow-500/30 text-yellow-400 bg-yellow-500/10' : ''}`}>{trainee.status}</Badge></td>
                      <td className="p-4"><div className="flex items-center gap-3"><Progress value={trainee.progress} className="h-1.5 w-24 bg-gray-800" /><span className="text-xs text-gray-500">{trainee.progress}%</span></div></td>
                      <td className="p-4 font-mono">¥ {trainee.revenue}</td>
          ...
          <AnimatePresence>
            {selectedTrainee && (
              <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 350, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="border-l border-white/10 bg-[#141418] flex flex-col">
                <div className="p-6 border-b border-white/10 flex justify-between items-start"><div className="flex flex-col items-center w-full"><div className="w-20 h-20 rounded-full p-1 bg-gradient-to-tr from-purple-500 to-pink-500 mb-3"><img src={TRAINEES.find((item) => item.id === selectedTrainee)?.avatar} className="w-full h-full rounded-full object-cover border-4 border-[#141418]" /></div><h3 className="text-xl font-bold">{TRAINEES.find((item) => item.id === selectedTrainee)?.name}</h3><p className="text-sm text-gray-400">Level 3 Producer</p><div className="flex gap-2 mt-4 w-full"><Button className="flex-1 bg-white text-black hover:bg-gray-200">{copy.detail.msg}</Button><Button variant="outline" className="flex-1 border-white/10">{copy.detail.profile}</Button></div></div></div>
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                  <div>
                    <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">{copy.detail.latest}</h4>
                    <div className="bg-black/40 p-3 rounded-lg border border-white/5"><div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 bg-cyan-900/20 rounded flex items-center justify-center text-cyan-400"><Disc className="w-5 h-5" /></div><div><div className="font-bold text-sm">Neon Nights.mp3</div><div className="text-xs text-gray-500">{copy.detail.submitted} 2h ago</div></div></div><div className="h-8 bg-gray-800 rounded flex items-center justify-center gap-1 opacity-50">{[...Array(15)].map((_, index) => <div key={index} className="w-1 bg-gray-400 rounded-full" style={{ height: Math.random() * 20 + 4 }} />)}</div><div className="flex gap-2 mt-3"><Button size="sm" className="flex-1 bg-green-600 hover:bg-green-500 h-8 text-xs">{copy.detail.approve}</Button><Button size="sm" variant="outline" className="flex-1 border-red-500/30 text-red-400 hover:bg-red-500/10 h-8 text-xs">{copy.detail.reject}</Button></div></div>
                  </div>
