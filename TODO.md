...
- [ ] **「导出画布」压缩包会静默丢掉所有图和视频**（v0.196 发现，**未修**）。
      ...
      —— 那句话是错的，当时没有实测。
- [ ] **下载的文件名由 OSS 对象名决定，不是我们给的名字**。跨域时浏览器忽略
      `a[download]`。v0.196 已保证**后缀正确**（按服务端改正过的 key 取），
      所以「下下来打不开」已解决；但拿到的是 `6b30f63…jpg` 而不是 `canvas-image-….jpg`。

**根治只有一条**：给 OSS 桶加 CORS 规则（允许五个 app 的域名，GET/HEAD，
`Access-Control-Allow-Origin`）。加完之后：导出压缩包立刻恢复正常、下载文件名由我们说了算、
`download-media.ts` 里的字节嗅探自动生效（现在跑不到）。**这是桶的配置，要在阿里云控制台改，
