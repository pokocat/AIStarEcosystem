...
- [ ] **`MaterialOpsE2ETest` 3 例断言失败，与素材产品库种子数据的 ID 解析有关（既有缺陷，非本轮引入）**（**留痕待排期**，2026-07-13）：`MaterialOpsE2ETest.getScript_returnsFullPayloadWithBlocks`（期望 `product_id=p4` 实得一个 19 位数字字符串）、`listVideos_filterByProduct` 与 `productLibrary_includesMaterialProducts`（均期望某记录 `id` 存在但查不到）在**干净的 `origin/main`（未做任何改动）单独运行该测试类**时即失败，说明不是测试间状态泄漏也不是本轮改动引入，而是测试自身对 seed 数据 `id` 的假设（字面量 `p4`/`video-2604-001`/`asset-2604`）与当前 seeder 实际生成的 ID（如日志里出现的 `3485332505048038713`，形似雪花/随机数字 ID）不一致，需要看 `MaterialOpsE2ETest.java` 建的测试夹具与实际 seeder（`DemoCatalogSeeder` 或同类）确认 `p4` 这个产品 id 是否已被上游改造为动态生成、测试夹具没跟着更新。定位：`apps/server/src/test/java/com/aistareco/aep/controller/MaterialOpsE2ETest.java:95,128,137,178`。
...
- [ ] **本条待新增回归测试**：`DramaReferenceAssetServiceTest` 补一例 mock `CreditService.commitHold` 在某个角度抛 `ResponseStatusException`（而非 `BusinessException`）时，验证 `releaseHold` 仍被调用、且最终抛出的异常能被 controller 层正确映射为 4xx（定位：`apps/server/src/test/java/com/aistareco/aep/service/DramaReferenceAssetServiceTest.java`）。
