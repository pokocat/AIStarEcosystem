...
- [ ] **`CelebrityStar.avatar`/`cover` 直接存 `FileStorageService` 返回的未签名公开 URL，未经 `CdnUrlSigner`，违反 §4.7.4 "DB 真值必须是 cdnKey" 硬规则**（记录待排期，2026-07-23 发现，暂不修）：`apps/server/src/main/java/com/aistareco/aep/model/CelebrityStar.java`（`avatar`/`cover` 字段，纯 `String` 无对应 `cdnKey` 列）；写入路径 `AdminCelebrityUploadController.java:75-77` 与 `StarProfileUploadController.java:80-83` 均直接把 `fileStorage.store(...)` 返回的稳定公开 URL 落库。当前代码注释里的技术理由（"非高带宽盗刷目标，公开 URL 可接受"）与 §4.7.4 的强制规定冲突——一旦运维按 §4.7.3 建议把 `AEP_CDN_SIGNED_URL_STRATEGY` 从 `oss` 切到 `cdn`，这两个字段会立即 403（不是 TTL 到期后才裂图，而是从来没签过名）。不在本轮修：需要新增 `avatarCdnKey`/`coverCdnKey` 列 + 改两个 upload controller 返回 key 而非 url + DTO 改经 `signer.signKey(...)` 派生，属于 §4.7.6 描述的多步迁移，风险与改动面超出例行 QA 单次 sweep 的范围，留给专门的迁移 PR。**2026-07-24 复核**：本轮独立审计再次排查，结论不变，仍留给下一轮排期（未新增变更）。

---

## 2026-07-24 · 例行 QA（未发现开着的旧 routine PR，1 处 Critical SSRF 高置信度新缺陷已修复）

> 开工前 `search_pull_requests`（`repo:pokocat/AIStarEcosystem is:pr is:open head:qa/routine`）+ `list_pull_requests`（state=open）均命中 0 条——上一轮 #97（`FinanceController` 缺鉴权 + 参考图装配 shotId 失效时静默用错镜末帧）已合并进 `main`（`bc72e67`），且 `git log bc72e67..origin/main` 为空，确认 main 自 #97 合并以来没有新提交——本轮是紧接上一轮的一次巡检，代码面没有变化，因此本轮把重点放在"挖此前可能被历次审计漏掉的旧代码路径"而非"新增代码里的新 bug"。
>
> 独立审计聚焦此前 20 余轮例行 QA 从未点名审计过的 mixcut 素材下载管线（此前的 SSRF 专项审计分别覆盖了 `PublishJobService`/2026-07-11、`DramaAssembleService`/2026-07-22 两条独立代码路径，但均未触达 mixcut），发现并修复以下 1 处：

- [x] ~~**`AssetDownloader.ensureLocal()`（mixcut 混剪素材下载）对用户可控的 `file_url` 零 host 校验，任意认证用户可发起 SSRF**~~（**已修复**，2026-07-24）：`MixcutRenderingService.resolveOne(JsonNode b)` 解析 `slot_bindings` 每条 binding 时，优先尝试 `asset_id → DB → localPath`，但 localPath 为空（例如通过 `ProductLinkPersistService`/`MixcutAssetService.registerExternalUrl` 登记的"外部 URL 素材"，`localPath` 恒为 null）时会退化为直接读**同一条 binding JSON 里客户端自己提交的 `file_url` 字段**（不再回查 DB 校验），随后原样传给 `AssetDownloader.ensureLocal(fileUrl)` 发起服务端 GET。而 `slot_bindings` 整体来自 `POST /api/mixcut/jobs`（`MixcutCreateJobRequest.slotBindings()`，`AepSecurityConfig` 对 `/api/mixcut/**` 只要求 `authenticated()`，任意登录用户可调）的请求体，前端 `create-client.tsx` 注释也证实"没素材时回退到 product.images[0]（外网 URL 直接作 file_url）"是被支持的合法形态——即这条字段被设计为可信任的客户端直传值，从未加过 host/地址校验。攻击者可构造 `slot_bindings: {"<slotId>": {"source":"upload","file_url":"http://100.100.100.200/latest/meta-data/ram/security-credentials/<role>"}}` 直接提交创建任务，服务端渲染 worker 会对该地址发起 GET 并把响应内容缓存落盘、喂进 ffmpeg 合成——構成可读取阿里云 metadata（临时 AK/SK）、扫描/探测内网服务的 SSRF，且产物最终会被合成进用户可下载的渲染视频里，存在数据外泄via视频画面的额外风险。与已修复的 `PublishJobService`（2026-07-11）/`DramaAssembleService`（2026-07-22）两处同类漏洞不同：mixcut 的这条路径**不能**简单收窄成"仅自身 CDN 域名"白名单——商品链接抓图是产品刻意支持的合法功能，需要下载任意公网 CDN 域的图片，收窄成固定白名单会破坏该功能。修复改用 SSRF 标准防护姿势：在 `ensureLocal()` 发起任何网络连接之前，按 URL 的 host 实际解析出的 IP 地址校验，拒绝环回（`127.0.0.1`/`localhost`）、link-local（`169.254.0.0/16`，含通用云 metadata `169.254.169.254`）、RFC1918 私网（`10/8`、`172.16/12`、`192.168/16`）、组播、any-local，以及 Java 内置方法未覆盖的 CGNAT 段 `100.64.0.0/10`（阿里云 metadata `100.100.100.200` 正落在此段）与 IPv6 ULA `fc00::/7`；DNS 解析失败一律按拒绝处理（宁可误杀不可放行不可控地址）。公网 CDN 域名解析出的公网 IP 不受影响，不破坏商品图片下载功能。新增回归测试 `AssetDownloaderTest`（14 例：9 个内部/metadata host 判定拒绝 + 2 个公网 IP 判定放行 + 3 个 null/空 host 拒绝 + 2 个 `ensureLocal` 端到端在发起任何网络连接前即抛 `IOException` 拒绝）。定位：`apps/server/src/main/java/com/aistareco/aep/service/mixcut/AssetDownloader.java`（`ensureLocal`/新增 `isBlockedHost`/`isBlockedAddress`）、调用链 `apps/server/src/main/java/com/aistareco/aep/service/mixcut/MixcutRenderingService.java:1001-1036`（`resolveOne`）、`apps/server/src/main/java/com/aistareco/aep/controller/MixcutController.java`（`POST /api/mixcut/jobs`）。

> 独立审计另确认（未发现新问题，聚焦此前专项审计较少触及的角落）：
> - 全仓其余服务端 HTTP 出站调用点（`DapMultimodalClient.download`/`DouyinHtmlScrapeHandler`/`MaterialOpsService.resolveSubmittedVideoUrl`/`MixcutOutputCleanupScheduler`/`CdnWebConfig`）逐一复核：`DouyinHtmlScrapeHandler`/`MaterialOpsService` 均已有 host 白名单（分别是 `jinritemai.com`/`douyin.com` 与已知短视频平台域名后缀集合），`DapMultimodalClient.download` 的 `url` 来自受信任的上游 AI 模型服务自身响应（非终端用户直接输入），`MixcutOutputCleanupScheduler`/`CdnWebConfig` 均为纯路径解析、无网络请求，均未发现新增 SSRF 面。
> - `window.confirm`/`alert`/`prompt` 全仓（含 `apps/admin`/miniprogram）：零命中裸原生调用。
> - CDN key-only discipline（§4.7）：未发现新增违规 `cdnUrl`-only 列或裸 URL 落库；`CelebrityStar.avatar`/`cover` 仍是唯一已知且已记录的欠账（见上条，结论未变）。
> - hold/commitHold/releaseHold catch 类型窄化：全仓搜索 `catch (BusinessException` 未命中任何 hold/commit/release 相关调用点，逐一核对与 2026-07-19/07-21 两轮一致，无遗漏。
> - `/api/admin/**`/`/api/me/**`/`/api/star/**` 下资金/积分相关写端点 `@PreAuthorize` 覆盖：逐一核对 `AdminCreditController`/`AdminUserController`/`AdminFinanceController`/`AdminFinanceRechargePackageController`/`AdminPaymentConfigController`/`AdminRechargeOrderController`/`AdminReconciliationController`/`AdminCreditOpsController` 均已收紧到 `FINANCE_ADMIN`/`SUPER_ADMIN`，与 2026-07-21 记录一致，无新增遗漏。
> - 复核 2026-07-15/07-18 记录的 4 处"已定位、本轮不修"缺口（`drama.credit.interactive-draft` 未暴露进 `/me/drama/config`、`shorts/make` 重复 `EditableField`、`/celebrity/videos/{videoId}` DELETE 缺失、小程序 `/celebrity/overview` 契约不匹配）：对照当前代码逐条重新核实，结论均未变，代码现状与描述完全一致，非过时记录，继续保留待产品侧排期。

**本轮复核历史 open routine PR**：无——`search_pull_requests`（`head:qa/routine`）与 `list_pull_requests`（`state=open`）均返回 0 条，无需 carry-forward，也无需 supersede/close 任何旧 PR。
**验证**：`./mvnw test-compile`（联网首次拉取依赖）全绿；`./mvnw -Dtest=AssetDownloaderTest test`（14/14 全绿）；`./mvnw -Dtest="Mixcut*Test" test` 广谱回归全绿（无失败）；`./mvnw test` 全量回归（进行中/完成情况见 PR 描述）。
