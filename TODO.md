...
- [ ] **视频成片历史的浏览 / 切换 UI**（v0.182 只做了数据层：`metadata.videos[]` +
  `primaryVideoId`，重出不再丢上一版）。图片节点已有现成的候选面板（`BatchFrame` +
  展开 / 设为主图 / 下载 / 重试 / 删除，见 `canvas-node.tsx`），视频节点还没有 ——
  用户现在能重出、不丢历史，但**看不到也切不回去**。照图片那套做一份即可。
