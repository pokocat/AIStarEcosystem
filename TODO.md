...
- [ ] **`drama_shorts` 幂等键升级为列 + 唯一索引**：`(owner_user_id, client_request_id)` 唯一索引才能挡并发同键双写（当前键存在 payloadJson，只挡顺序重试）。**已不再被阻塞**（下条误报已撤回）：新迁移编号从 **V23** 起；新列全为 NULL，MySQL 唯一索引允许多个 NULL，加索引不会因存量数据失败。
...
- [ ] **防再次误判：迁移编号真源要一眼可见**（本轮教训）：编号横跨两个目录（`resources/db/migration/*.sql` + `java/db/migration/*.java`），只看 SQL 目录会看到「V1 跳到 V14、停在 V20」这种假象——这正是本轮误报和历史「V20 撞号」的同一个坑。建议在 SQL 目录加一个 `README.md`（或在 V1 顶部加注释）写明：编号两处一起数、下一个可用编号怎么查（线上 `flyway_schema_history` 最大 version + 1）。
