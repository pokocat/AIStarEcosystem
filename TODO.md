...
- [x] **D-10 分镜一致性优化**（v0.97，2026-06-30，借鉴 [HKUDS/ViMax](https://github.com/HKUDS/ViMax)；真源 [`docs/drama-storyboard-consistency.md`](docs/drama-storyboard-consistency.md)）：P0/P1/P2 全量落地。
  ...
  - [ ] **后续可选**：VLM best-of-N 首帧一致性自检；`lastFrameUrl` 末帧 CDN 镜像（当前存上游 URL，best-effort 有时效）。
- [ ] **D-11 出片真·多模型选择**（v0.98 拆出，独立 PR）：当前一个用途只能绑一个端点（`AiAppBinding` 主键即 `purpose`，music/celebrity/drama 共用），故「选模型」无法真生效。要做真的：① 用途→N 候选端点（新表或 scope）+ `AiModelInvocationService.resolveEndpoint(purpose, endpointId)` 重载（白名单校验，否则回退默认，§8.0 选的端点未配 → 503 不静默回退）；② `GET /me/drama/render/models` 候选列表；③ `RenderClipInput`/controller/service 透传 model/endpointId；④ admin「AI 应用绑定」支持 VIDEO_GENERATION 多选；⑤ 按模型差异化单价。改共享 AI 绑定基建，单独排期。
- [x] ~~**D-12 短视频草稿资产 URL 重签**（v0.98 发现）：`DramaShort.payloadJson` 存的是当时签名的 OSS URL（首帧/末帧/成片），`AEP_CDN_SIGNED_URL_TTL_SECONDS`（默认 3600s）过期后 403 图裂——与 `DramaProject` 同类债。`DramaProject` 已在 `toDetail` 递归 `signer.maybeSign` 重签修复（v0.98，见 AGENTS §4.7.7）；`DramaShort` 出 wire 漏斗需照做（注入 `CdnUrlSigner` + 复用 `resignAssetUrls` 范式）。~~ **审计确认，2026-07-01（例行 QA 巡检）**：本条描述已过时——`DramaShortService` 早在 v0.76 补丁（commit `81d9030`，2026-06-17，"re-sign short-draft frame/video URLs on reopen"）就已注入 `CdnUrlSigner` 并在 `toDetail` 里加 `resignPayloadAssets`（遍历 shots 的 `frameUrl`/`frameUrls`/`videoUrl` 走 `resolveAssetUrl`→`signer.maybeSign` 重签），`DramaShortServiceTest` 当时已加断言覆盖。9f139da（v0.98，2026-07-01）修 `DramaProject` 同类问题时误判 `DramaShort` 尚未修复而补记本条——先审计代码再排期，避免重复劳动。
