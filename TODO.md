...
- [ ] **人物 / 场景只能改不能增删（原条目）**：`/shorts/prompt` 预览页与 `/shorts/make`「提示词设定」卡都只支持编辑既有人物卡与场景（`ShortDraftData.visualBible`），想加一位角色或一个场景只能改原文重拆。补一组增删按钮（新增时给空 `visual` 并提示「不填外观则该角色不参与画面锚定」）。
...
- [ ] **分镜表不显示出场人物（原条目）**：`ShortStoryboardTable`（`/shorts/make` 的逐镜工作面）没有 `castNames` 列，进工作台后只能在「提示词设定」里改设定、不能改某一镜挂谁；且「加一镜」新建的镜头没有 `castNames` 字段 → 服务端按「未标注」全员锚定（见 `DramaShortContinuityService.ensureDraft`）。加一列人物 chip（与预览页同交互），并让「加一镜」显式落空数组或继承上一镜。
