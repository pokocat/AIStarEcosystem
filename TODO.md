...
- [ ] **D-10 分镜一致性优化**（借鉴 [HKUDS/ViMax](https://github.com/HKUDS/ViMax)；真源 [`docs/drama-storyboard-consistency.md`](docs/drama-storyboard-consistency.md)）：
  - [x] **P0 镜间承接**（v0.97，2026-06-30）：视频工厂出首帧时把 `ref_images` 扩展为「角色参考图 + 场景参考图 + 同场上一镜帧」（`stages/factory.tsx` `prevSceneFrame`/`shotRefImages` + 「镜间一致性承接」开关）。纯前端、零契约。门禁 typecheck+build 绿。
  - [ ] **P1 文本层增强**：把 ViMax 电影语言规则（叙事目的 / 机位复用 / 画面位置 / 角色特征）补进 `drama.epscript.md` + `drama.split_scene.md`；`BoardShot` 加 `camId?`（机位载体）+ `normalizeShot` 透传 + epscript/split_scene JSON 模板加字段。需 openapi/PRODUCT 同步。
  - [ ] **P2 视频层关键帧 i2v**：① 新 prompt key `drama.decompose`（出 ff_desc/lf_desc/motion_desc/variation_type + 角色 id 存在性校验，复用 `MaterialAiService` 重试）+ 计费 `drama.credit.decompose`；② 末帧出图；③ `MaterialVideoModelClient` 加 `PROTOCOL_SEEDANCE`（`content` 数组 role=first_frame/last_frame + `return_last_frame:true`，火山方舟形态）—— **注意当前 seedance 落 GENERIC 分支连首帧都没传**；④ `return_last_frame` 回填 `BoardShot.lastFrameUrl` → 下一镜首帧参考（链式承接闭环）；⑤ `/render/clip` body 加 `last_frame_url?`/`return_last_frame?`（openapi + BUSINESS_RULES 同步）。下游模型不支持时字段被忽略不报错（§8.0：传入不生效 ≠ 静默伪造）。
