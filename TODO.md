...
## 2026-08-29 · v0.136 音乐自由创作 + 工作室惰性补建后续

- [ ] **追查历史账号漏建 Studio 的注册路径**：线上 17 个用户里 6 个 STUDIO 账号无 `aep_studios` 行（含 2026-05-25 注册的 `phone_18801931018`，追查号 7MQUAP4JYVSE 409 死锁的触发者）。正常短信+激活码注册经 `LicenseActivationService.activate` 必建 Studio——这 6 个账号从哪条路径来的（早期版本？admin 手建？）未定位。v0.136 的 `DigitalIpService.ensureStudioFor` 惰性补建已止血，root cause 供审计排期。
- [ ] **无艺人歌曲的「补绑定艺人」入口**：v0.136 起自由创作歌曲 `artistId=null`，产品语义是分发前必须补绑定（product_spec.md §10.1），但目前没有绑定 UI/端点（可做 `PATCH /me/songs/{id}` 允许补 `artistId` + 前端歌曲详情入口）；分发链路也未加「未绑定艺人 → 拦截并引导」的闸。
...
- [~] `clip` 多段总装继续收尾。v0.112 已完成 Strategy A 基线；v0.113 又补齐三套模板各自的运行时固定品牌尾卡（不再交付空白色块）、最终音轨 -16 LUFS / -1.5 dBTP 归一、平均亮度/综合响度/真峰值失败关闭和真实 ffmpeg 解析探针。**剩余**：拿到授权的「集体发声」群像尾片后通过 admin preset 替换品牌尾卡，并用本人真实长片完成时延、内存、磁盘与阈值压力验收；完成前仍不能宣称完整生产模板成片。
