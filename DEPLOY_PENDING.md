# 待部署：clip service token 运营面（40a4e979）
...
DEPLOY_HOST=ecs-user@47.98.162.120 SSH_KEY=~/dev/aliyun/aiartist.pem \
  RELEASE_ID=20260911033644-40a4e979 \
  ./infra/scripts/deploy-release.sh dist/deploy/20260911033644-40a4e979 server
```

包在 `dist/deploy/20260911033644-40a4e979/`（app.jar 127MB）。内容是 40a4e979 那个提交：
新增 `/api/service/clip/admin/**`（service token 鉴权的模板列表 + 上下架 + 供应商总览）、
`ClipTemplateService.setStatus`、SecurityConfig 放行一条路径。**纯新增，不动既有接口，
没有 Flyway 迁移。**
...
军师 BFF 侧（8.136.36.175）已经发到 `092755c`，不用再动。
