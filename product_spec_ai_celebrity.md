...
- **明星入驻**：web-star `/onboard` 创建 `CelebrityStar` + `StarAccount` 绑定 → 明星市场（`/celebrity/stars`）立即可见。
- **带货授权闭环**：web-celebrity 申请授权页改真实提交 `POST /api/me/celebrity/stars/{id}/authorization/apply`（`CelebrityStarAuthorization` upsert→PENDING，新列 `applicantNote`）→ web-star「带货授权」审批 → 批准/驳回即时反映到本线明星详情授权块 + 站内通知申请方。注意：admin 的 star-authorizations 管理页与明星端审批共用同一张表 / 同一状态机。
- **商品报备**：本线商品库行内新增「报备」（`StarFilingDialog`，仅已授权明星可选）→ `POST /api/me/celebrity/products/{id}/star-filings` 创建 `StarProductOnboard`（source=creator，落 step=2 明星审核；公共商品池视为已过平台初审）→ 明星端 6 步入库（双路寄样）→ `GET /api/me/celebrity/star-filings` 本线回查 stepLabel；重复报备 409。
- 平台体系：`SubProduct`/`PlatformSupport` 加 `star`；明星端种子账号 `star_shenteng / star123`（绑沈腾）。

明星端自身 13 模块（IP 资产 / 报白 / 数字人 / AI 形象 / 内容审核 / 商品入库 / 商品库 / 品牌授权 / 收益 / 规则 / 侵权 / 合同）规格见 [`apps/web-star/PRODUCT.md`](apps/web-star/PRODUCT.md)；增量细节见 [`docs/VERSION_HISTORY.md`](docs/VERSION_HISTORY.md) `### v0.60`。
