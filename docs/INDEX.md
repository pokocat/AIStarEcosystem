...
| [`docs/drama-short-vimax-lite-plan.md`](drama-short-vimax-lite-plan.md) | AI 短剧（风格短片） | ViMax-lite 前后一致性提升计划：提示词去污染 / ContinuityManifest / 关键帧依赖 DAG / 七牛 Kling 一致性模式 / 确定性声音总装 / 零 Token QA；明确不做多 Agent Review（last-reviewed 2026-08-18） |
| [`docs/[Fabel5][TODO]drama-audit-2026-07.md`](%5BFabel5%5D%5BTODO%5Ddrama-audit-2026-07.md) | AI 短剧（全局审计） | 产品逻辑 × 技术架构审计（只列问题+改进方案）：一致性引擎五层架构（L0 实体化→L3 VLM 质检）+ C-序列路线图 + P0-P3 排期总表；供后续执行 agent 作工作输入（last-reviewed 2026-07-04） |
...
| [`docs/ip-ecosystem-integration.md`](ip-ecosystem-integration.md) | **IP 工作台 / 数字资产 / 数字名片 整合方案** | 一条链：造形象 → 登记资产 → 对外发布。§2 打通只靠 `dapDisplayRef` + `DapAssetUsage` 两样现成东西、§3 内置工作流模板「IP 打造」（装扮 4 / 表情 6 / 短动作 3，性别只决定服装品类与体态基准、不决定表情性格）、§4 用户动线、§5 需新增项、§6 三条红线（last-reviewed 2026-09-06） |
| [`docs/digital-business-card-plan.md`](digital-business-card-plan.md) | **AI 数字名片（`apps/web-aiavatar` 下 `/card` 域）方案真源** | 见客户递一条链接就把人说清楚。一期只做门面（扫码即看 / 形象三档 / 存通讯录），交换与名片夹挂二期。§3 不发新产品码（`/api/v1/**` 兜底到 aiavatar）、§5 形象走 `dapDisplayRef` 引用不拷贝、§6 与 dap / ipstudio 的职责边界、§8 视频层叠三方案与交付清单、§9 需新增项（last-reviewed 2026-09-06） |
| [`docs/ip-studio-generalize-proposal.md`](ip-studio-generalize-proposal.md) | **IP 工作台通用化提案（未定案）** | 形象卡五字段（outfit/pose/expression/details/props）在服务端只是按序拼成一串文本，拆分对出图无增益；建议改为「一个提示词框 + 内置模板库」，节点 7→6 只删 `look`，五层一致性锁定与计费纪律不动，零迁移（last-reviewed 2026-09-06） |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.129） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
