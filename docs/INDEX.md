...
| [`docs/clip-avatar-video-plan.md`](clip-avatar-video-plan.md) | **`clip` 口播视频线**（军师小程序分包） | v0.116 已对齐石榴官方采集/错误/进度契约，增加服务端 ffprobe 采集闸和客户端 requirements；声音/数字人统一走 V2 段音频驱动，多段总装、固定尾卡与音画质量门已落地。Scheme A 仍由军师扣积分；媒体机器审核、本人素材质量实测与真实发布继续失败关闭（last-reviewed 2026-08-11）。 |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.116） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
