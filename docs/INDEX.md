...
| [`docs/clip-avatar-video-plan.md`](clip-avatar-video-plan.md) | **`clip` 口播视频线**（军师小程序分包） | v0.126：单视频直创与可选声音增强保持不变；多句可共用同一画面，但字幕保留原句 cue 并按口播时长逐句切换。普通素材以 `previewUrl` 加载轻量封面、以 `contentUrl` 在用户主动点击后播放原视频或查看大图。声音 `>2s`、形象视频 `>=5s`，V2 段音频驱动、多段总装、固定尾卡与音画质量门已落地（last-reviewed 2026-08-11）。 |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.126） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
