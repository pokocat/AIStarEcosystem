...
| [`docs/clip-avatar-video-plan.md`](clip-avatar-video-plan.md) | **`clip` 口播视频线**（军师小程序分包） | v0.127：支持多数字人、已有声音复用和项目精确选择；模板固定片段由后台 preset asset 配置，名称/秒数/封面/视频与创建项目同源；作品缺缩略图可从成片补抽。单视频直创、逐句字幕、多段总装与音画质量门保持不变（last-reviewed 2026-08-11）。 |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.127） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
