...
| [`docs/clip-avatar-video-plan.md`](clip-avatar-video-plan.md) | **`clip` 口播视频线**（军师小程序分包） | v0.118 对齐石榴直传创建：`authId` 仅为可选校验项，普通用户按“先克隆声音 → 上传形象视频 → 训练”完成创建，不再被额外授权视频硬闸阻断；声音 `>2s`、形象视频 `>=5s`，较长时长只作软建议。声音/数字人统一走 V2 段音频驱动，多段总装、固定尾卡与音画质量门已落地（last-reviewed 2026-08-11）。 |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.118） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
