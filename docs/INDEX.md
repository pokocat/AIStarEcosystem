...
| [`docs/clip-avatar-video-plan.md`](clip-avatar-video-plan.md) | **`clip` 口播视频线**（军师小程序分包） | v0.122：单视频直创与可选声音增强保持不变；形象视频与普通视频素材都有独立 JPEG 预览，历史记录可按源视频补图，临时文件名不再进入产品 UI，删除同步清理源素材和缩略图。声音 `>2s`、形象视频 `>=5s`，声音/数字人统一走 V2 段音频驱动，多段总装、固定尾卡与音画质量门已落地（last-reviewed 2026-08-11）。 |
...
| [`docs/VERSION_HISTORY.md`](VERSION_HISTORY.md) | 完整连续版本增量（v0.5 → v0.122） | 新实体 / 路由 / 决策 / 注意事项的全量历史；从 AGENTS.md 拆分以保持主文件轻量 |
