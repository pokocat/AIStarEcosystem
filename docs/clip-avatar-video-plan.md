...
- v0.118 对齐石榴 Train Avatar Model 的真实可选项：`authId` 仅在业务确实需要授权视频校验时填写，不填写默认不校验。普通创建不再经过 `CLIP_CONSENT_REQUIRED`，默认顺序为“先克隆声音 → 上传形象视频 → 云端训练”；历史账号已有 authId 时仍兼容携带。v0.117 的时长硬门保持为声音真实 `>2s`、形象视频 `>=5s`，8–15 秒声音、10–20 秒形象仅为质量建议。
- v0.117 隔离预发版本 `b5140a8a-20260811T132756Z` 已关闭 force-mock：服务 active、`NRestarts=0`，3 模板通过；AIStar 与公网军师 BFF 的 requirements 均返回形象硬门 5 秒、建议 10–20 秒，声音供应商硬门 2 秒/端上硬门 3 秒、建议 8–15 秒。自动化没有创建任何计费任务，下一步由用户本人从军师预发真机包提交素材。
...
| **M1 · MVP** | 3~4 周 | clip 域 server（走 §5 新增领域 SOP 六步 + openapi 契约）；克隆向导（声音 → 形象 → 训练）；三步出片全链；《为实体发声》单模板；H5 内测 | 从进模板到拿成片 **< 5 分钟**；四门编译全绿 |
...
3. 模板详情：竖屏效果预览 + 结构拆解图（哪些句子分身出镜、哪些配画面、结尾
   固定段）+ 积分价格 + 「开始制作」
4. 数字分身克隆向导（分步）：① 声音克隆（页面内照稿录音或上传音频）
   ② 形象采集（直接拍摄/上传视频，带拍摄要点与素材使用权确认）③ 提交训练 →
   训练中状态页（预计时间、可先用平台预置形象）；普通创建不另录授权视频
