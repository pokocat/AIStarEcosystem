...
> **文档状态**：**M1 服务端与石榴真实原子链路已落地并部署隔离预发；完整多段总装、媒体审核、代发未上线**。
...
- 非 mysql/production 环境允许显式 mock，mock 产物带 `mock=true`。媒体机器审核未配置时军师 BFF 继续 fail-closed；当前 assembly 只持久化石榴返回的单一主口播视频，尚未实现模板所需多段 avatar/b-roll/尾片 ffmpeg 总装；真实代发仍固定失败。
- 仍需使用本人合规素材完成 §3.2 质量/时延/一致性/规格/成本实测，完成 §12 商务/备案决策，并接媒体审核、完整总装、成片内 AI 标识、四平台发布和生产压测/真机验收。
