...
- v0.117 纠正采集时长门槛：`/me/clip/avatar/requirements` 的硬门只按石榴官方资源限制执行——声音真实时长必须 `>2s`（端上按整秒提示至少 3 秒），授权/形象视频必须 `>=5s`；8–15 秒声音、10–20 秒形象仅为可跳过的质量建议。v0.116 将渲染策略收口为「所有非尾段先由同一 V2 speaker 生成音频，avatar 再以 `/video/createByVoiceV2` 驱动」，不再混用 `createByText` 内嵌 TTS；授权 `authId` 只表达上游受理声明，不冒充实名认证。
