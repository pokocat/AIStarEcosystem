...
`WorkDto` 新增 `createdAt/generatedAt`：开始时间取最新生成任务的创建时间，完成时间优先取任务 `completedAt`，因此发布或项目最后编辑不会篡改成片时间；旧终态任务缺完成时间时只用任务/项目更新时间兼容补齐。军师作品卡据此显示到分钟，生成中与已完成语义分开。
...
字幕绘制不再对单句强制两行省略，逐句分层后可以完整换行。定向测试覆盖三句共用画面的 cue 保留，以及两句共用画面时生成两个不重叠时间窗；测试媒体模式同样走该字幕时间轴，不调用石榴、不消耗点数。

隔离预发已发布 `3560b942-20260811T170602Z`，`AEP_CLIP_FORCE_MOCK=false`；服务 active/running、`NRestarts=0`，三套模板探针通过。发布与验收没有上传素材、没有创建声音/形象/视频任务。

### v0.125（2026-08-11）— 素材原媒体可主动预览

`AssetDto` 新增 `contentUrl`，由 `ClipAsset.cdnKey` 生成原始媒体短期签名地址；`previewUrl` 继续只返回图片原图或视频 JPEG 抽帧，供列表轻量加载。军师素材库因此可以显示真实封面，并在用户主动点击后播放视频或查看大图，而不是把缩略图误当成完整素材。

服务测试锁定视频上传同时返回 JPEG `previewUrl` 与 MP4 `contentUrl`。本变更不改存储结构、不复制媒体，也不触发石榴任务。
