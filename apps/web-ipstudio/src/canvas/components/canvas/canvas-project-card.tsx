...
// 上游是 react-router，本仓是 Next App Router —— 只有这一处路由耦合。
...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const open = () => {
        ...
        const href = `/canvas/${project.id}${searchParams.toString() ? `?${searchParams.toString()}` : ""}${agentHash}`;
        if (agentHash) router.replace(href);
