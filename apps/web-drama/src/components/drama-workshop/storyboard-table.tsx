...
import { ArrowUp, Check, Clapperboard, Film, Image as ImageIcon, Play, Plus, RefreshCw, Sparkles, Wand2, X, Zap } from "lucide-react";
    ...
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      ...
      {edit && (
        <AiImageEditModal
          ...
          onCommit={(url) => { props.onFrameEdited(edit.sceneId, edit.shot.id, url); }}
        />
      )}
    </div>
  );
...
/** AI 改图弹窗 — 设计稿 imgEditOpen：左指令对话 + 右 9:16 预览 + 版本号；复用 render/frame + ref 图迭代。 */
const EDIT_CHIPS = ["换成夜景", "让她回头", "换暖色调", "背景虚化", "再高级一点"];
function AiImageEditModal({ shot, onClose, onCommit }: { shot: FormShot; onClose: () => void; onCommit: (url: string) => void }) {
  const [msgs, setMsgs] = React.useState<{ role: "ai" | "user"; text: string }[]>([
    { role: "ai", text: `这是镜 ${shot.no} 的首帧。想怎么改？直接说，比如「换成夜景」「让她回头」「再高级一点的色调」。` },
  ]);
  const [input, setInput] = React.useState("");
  const [url, setUrl] = React.useState<string | undefined>(shot.frameUrl ?? shot.frameUrls?.[0]);
  const [ver, setVer] = React.useState(1);
  const [busy, setBusy] = React.useState(false);
  const scrollRef = React.useRef<HTMLDivElement>(null);
  React.useEffect(() => { const e = scrollRef.current; if (e) e.scrollTop = e.scrollHeight; }, [msgs, busy]);
  ...
  const send = async (text: string) => {
    ...
    try {
      const frames = await RenderApi.renderFrame({
        kind: "shot",
        vars: { desc: `${shot.visual || "分镜画面"}。改图要求：${t}` },
        ratio: "9:16",
        count: 1,
  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="pop-in col" onClick={(e) => e.stopPropagation()} style={{ width: "min(900px,94vw)", height: "min(580px,90vh)", background: "var(--surface)", borderRadius: 20, overflow: "hidden", boxShadow: "var(--shadow-lg)" }}>
        <div className="row gap-2" style={{ padding: "14px 18px", borderBottom: "1px solid var(--line-soft)", alignItems: "center" }}>
          ...
          <span className="tag tag-accent" style={{ flex: "none" }}>镜 {shot.no}</span>
          <span className="grow" />
        ...
        <div className="row grow" style={{ minHeight: 0, alignItems: "stretch" }}>
          ...
          <div className="col center grow" style={{ padding: 22, background: "var(--surface-2)", minWidth: 0, gap: 12 }}>
            <div style={{ position: "relative", height: "100%", maxHeight: 430, aspectRatio: "9/16", borderRadius: 14, overflow: "hidden", boxShadow: "var(--shadow)", background: "#1c1917" }}>
              {url ? <img src={url} alt={"镜" + shot.no} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div className="col center grow" style={{ height: "100%", color: "#fff" }}><Sparkles size={28} /></div>}
              {busy && (
