...
import { ArrowLeft, Clapperboard, LogOut, Sparkles } from "lucide-react";
import { useAuth } from "@ai-star-eco/api-client";

export default function DramaConsolePlaceholder() {
  const { user, logout } = useAuth();
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 h-[480px] w-[1100px] bg-gradient-to-br from-orange-500 via-amber-500 to-red-500 opacity-[0.12] blur-[120px] rounded-full"
      />
      <header className="relative z-10 flex items-center justify-between px-6 md:px-12 py-5 border-b border-white/5">
        <Link href="/" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-orange-500 via-amber-500 to-red-500 flex items-center justify-center shadow-lg">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-6 md:px-12 py-20 md:py-28 text-center space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.04] text-orange-300">
        ...
        <p className="text-base text-gray-400 leading-relaxed">
          演员 IP、脚本工坊、短剧项目与分发模块即将搬入本独立 app。
          当前阶段（Phase 4b）已完成明星带货专区的迁移，短剧工作台计划在
          后续 Phase 完成。
        </p>
