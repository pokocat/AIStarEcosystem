...
// JSON 接口 USE_MOCK 由 src/mocks/_handlers/appearance-forge.ts 拦截。
// streamForgeConversation 为 SSE 流式接口，registry 不拦截 stream，mock 仍内嵌于此。
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * 执行一次锻造生成。后端预期为异步任务（返回已完成的 ForgeResult 或发起轮询）。
 */
 ...
 *
 * 当前 AI 视频生成尚未接入：
 * - mock 模式下从 DEMO_FORGE_VIDEO_POOL 随机挑一个 URL 写入 videoUrl。
 * - 真实后端 `POST /api/appearance-forge/save` 行为一致，URL 由 server 端
 *   {@code ForgeController.DEMO_VIDEO_POOL} 维护。
 * 接入真实 AI 后，后端应替换为触发生成任务并回填真实 videoUrl。
 *
 * @param result 要保存的 ForgeResult
...
export async function getForgeProviderStatus(): Promise<ForgeProviderStatus> {
  if (USE_MOCK) {
    return { configured: true, provider: "mock", message: "当前为演示模式，将使用本地回放生成方案" };
  }
  return apiFetch<ForgeProviderStatus>("/appearance-forge/chat/status");
}

/**
 * SSE 流式锻造对话。registry 不拦截 stream — 当 USE_MOCK=1 时由本函数内的
 * streamMockForgeConversation 直接生成事件流。
 */
