...
/shorts/prompt       ← 提示词直出（v0.143:粘贴写好的整段提示词 → 拆成人物卡/场景/画面基调/逐镜分镜 → 预览改 → 开拍）
...
### v0.148 · 2026-09-02 · 全站文案去 AI 味 + 「近期热点」快捷输入收口（纯前端，无端点 / 无实体变更）

- **退役阶段名清出界面**：`视频工厂` 阶段 v0.98 已删、流程从六阶段收为五阶段，但 9 处用户可见文案仍在指路（素材库「可在视频工厂 @ 引用」、左轨「脚本 → 视频工厂 → 成片」、互动剧「前往六阶段制作视频」等），互动剧那条会把用户导向一个进不去的地方。统一改为真实阶段名（`剧集脚本` / `成片合成` / `剧集工作台`）。`scripts` 页的「我的短剧 → 进入项目 → 单集剧本」三个名字全是旧的，一并改正。
- **一处一个名字**：侧栏叫「创意市场」而发布流程全叫「创意中心」、「短视频工坊」在模板页叫「短视频工厂」——统一到侧栏叫法（含两个断言该字符串的测试）。侧栏分组 `提效` → `创意`。
- **无出处断言按 rewrite-safe 删除**：`经验证的创作配方` / `一套验证过的创作配方` / `选择经过验证的热门结构`——页面没有给出验证主体与口径，删掉背书层，只留能独立成立的部分。
- **黑话与拔高**：`创意中台` / `闭环短剧生意` / `人物档案统一治理` / `产品矩阵`（且链到本页）/ `跨项目 IP 资产` / `才艺矩阵` / `孵化营 · 在岗` / `顶级 IP` / `一键孵化` / `敬请期待` / `开箱即用` / `2026 影视年` 等逐处改回普通说法；运营表单漏出的规格语言（`去具体化的可迁移主线，作为套用者大纲生成的输入`）改成人话。
- **§4.6 中文单语**：`eyebrow="parameters" / "results" / "versions" / "actions" / "wardrobe & props"` 与 KPI `ARPDAU` 改中文。
- **「近期热点」快捷输入**：三处（首页 / 新建短剧 / 新建短视频）此前各写一遍相同 markup，条数策略却不同（随机 3 / 前 4 / **不限**），新建短视频那处会把运营配的十几条整句钩子全铺出来（线上实测 10 条铺 5 行）。抽出共用 `HotTopicChips`，统一条数上限并都改为每次进页随机取一批。
- **chip 只显示短标签**：数据结构本是 `label` 上 chip（短）+ `idea` 点击填入（整句），但 `/operations` 的「AI 生成一批」写的是 `{ label: c, idea: c }`——整句当标签。新增纯函数 `hotTopicLabel`（取第一个分句、超 12 字截断，6 个 vitest）在生成入口与渲染层各兜一次：**渲染层这一兜让库里已存的整句数据无需运营重新编辑就能正常显示**；整句仍完整保留在 `idea` 与 hover。chip 另加 `maxWidth` + 省略号（§8 不溢出）。
- **内置默认热点刷新**：原 3 条含 `世界杯看台被拍` 等绑定具体时事的条目，换成 6 条不绑时事的常青短剧设定，并在注释里写明这类默认值不要再挂具体事件。注意这只是**后端未配置时的回退值**；运营已在「内容目录」配了 override 时不生效，需在该页点「恢复默认」。
- **热点蒸馏 prompt 重写（同版补，改 server）**：线上产出过 `包书皮时意外发现，前任留下的惊天伏笔`（学生开学话题 × 成人恋爱话题硬焊）、`军训偶遇武状元`（现代 × 古代）、`贴秋膘吃出百万身家`（因果不成立）、`想一万次不如做一次，逆袭从此刻开始`（纯口号）这类不成立的选题。根因：旧 prompt 只要求「改写成钩子」，从未约束前后半句属于同一个人、同一个世界，模型于是把热词与爆款套路机械拼接。重写 `drama.hotspot_distill.md`：热词只供**情境**，冲突必须是该情境里该人真会碰上的事；动笔前先填「谁 / 情境 / 冲突」三格，填不满即丢；五类错配（话题域 / 世界 / 因果 / 口号 / 空心套模板）各配一条**线上真实坏例**作反面样例；自查方式明确为「删掉『竟/惊天/从此』后能否立住」；配额从「最多 12 条」改为「0–8 条都正常，凑数比少给更糟」。`DramaHotspotService` 默认 temperature 0.85 → 0.65（该任务要语义一致而非发散，创意度来自输入热词；运营仍可在后台覆盖）。`PromptTemplateSeeder` SEED_VERSION → `v11-2026-09-02-hotspot-coherence`，只刷新运营未改过的行（version==1）。
- **v11 收得过紧，同日修正为 v12**：v11 上线后线上连续 6 次产出 **0 条**（同一批 50 条热词，旧提示词能出 10 条），运营侧只看到 502 `AI_BAD_OUTPUT`「没能从热搜里蒸馏出合适的短剧选题」。两个独立原因：① 提示词把 5 条反面样例 + 「谁/情境/冲突」三格硬门 +「凑数比少给更糟」叠在一起，模型最安全的选择变成什么都不出；当天热搜以台风、泥石流等硬新闻为主，丢弃规则又被读成「放弃整批」。② **代码与提示词自相矛盾** —— 提示词明说「可以返回空数组」，`DramaHotspotService` 却把空数组判成 502，前端本就有的「这次没生成出可用选题，换个时间再试」友好分支因此永远走不到。修法：提示词改为「逐条判断、不要整批放弃、通常 4–8 条」，并说明丢弃规则只针对那件事本身不该被娱乐化；`parseHotspots` 区分「输出不可解析」（返回 null → 502）与「合法空数组」（返回空 list → 200，交前端如实展示）；两种情况都记录模型原文前 400 字 —— 原实现产出为空时不留证，只能看到「没产出」，无从判断是热词不可用还是提示词过严。SEED_VERSION → `v12-2026-09-02-hotspot-no-collapse`。
- **未改**：抓取链路本身（抖音热搜源实测 HTTP 200 / 50 条正常）。该功能仍是**运营手动触发、无定时任务**，不点就一直是同一批。

### v0.143 · 2026-08-31 · 短视频「提示词直出」入口

- **第二条入口**：此前短视频只能经 AI 创意 / AI 对话生成（一句话 → AI 创作）。新增 `/shorts/prompt`：把用户自己写好的整段提示词（人物设定卡 + 场景光影 + 全片基调 + 带时间码的分镜脚本）**忠实拆解**成人物卡 / 场景 / 画面基调 / 逐镜分镜，预览页可逐项修改后开拍。入口在短视频工坊头部按钮、工坊新建卡（与「一句话生成」并列）、以及新建控制台底部一行。
- **后端**：`POST /me/drama/shorts/parse-prompt`（新 promptKey `drama.short_prompt_parse`，复用 `DRAMA_SCRIPT_DRAFT` 端点；免费、不落库，与脑暴 chat/outline 同惯例）；`POST /me/drama/shorts` 支持 `body.seed`，一次开拍费即建成带分镜的草稿。§8.0：端点/提示词未配置 503、调用失败 502、输出不可解析 502，绝不用正则或模板兜底拼假分镜。
- **设定真的进模型**：`ShortDraftData.visualBible`（人物卡 / 场景 / 全片基调）由服务端在保存与预检时派生成一致性锚点（多角色 `character-main/-2…`、多场景 `scene-main/-2…`），每镜按 `castNames` / `sceneName` 精确挂人挂景；人物「外观」进逐镜画面，「表演」只用于配音，互不污染。
- **边界与如实回报**：提示词 20–20000 字；单镜 2–15 秒（超长收口）、整条最多 40 镜（超出不拆解），这些处理一律写进 `notes` 并在预览页与工作台展示，不静默改用户设定。
- **工作台**：`/shorts/make` 新增「提示词设定」卡（可改人物卡 / 场景 / 基调）+「原始提示词」查看与复制 +「按提示词重拆」（带确认，可在左侧对话里说要怎么调），提示词直出线不再走主题式 AI 重写以免丢设定。
...
- **版面顺序对齐设计稿**：短视频大纲卡在上（含 beat 流），「分镜表」段标题（共 N 镜 · 约 N 秒 + 文字可直接改 + 重新生成 + 一键连跑出片）+ 平铺表在下。
- **大纲卡贴合设计稿**：标题升为「一句话概述」描述位（无边框大字，可直接改）+ beat 流；风格 / 主场景 / 主角收进「更多设定」折叠区（默认收起，仍可编辑、喂提示词）。
