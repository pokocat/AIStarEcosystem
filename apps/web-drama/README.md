...
/interactive         ← (v0.74 起整合进短剧工坊 · 旧链接重定向 → /projects?filter=interactive)
/interactive/[id]    ← 互动剧编辑器（剧集分支图/画布 + 生成 + 导出；从短剧「转换」或顶部「新建/AI起草」进入）
...
- **编辑体验完善**：自动保存（复用 v0.76 草稿状态机 `useSaveStatus` + `SaveStatus` 指示 + 离开提醒，
  替代手动保存按钮）；生成面板显示预计积分消耗（按 `drama.config` 的 clip 单价 × 待生成集数）+ 分支
  过多提示；孤立节点在卡片上就地标「孤立 · 走不到」；支持「复制本集」快速做分支变体。
- **可视化分支编排（图/列表双视图，`BranchCanvas`）**：剧集图按分支深度 BFS 自动左→右铺开，SVG 贝塞尔
  连线把互动选项文案标在线上，一眼看清剧情走向；起始/结局/孤立节点分别高亮，节点带生成态圆点。
  点节点 = 编辑；点节点的「拉线」手柄 → 再点目标 = 直接接一条分支（已是互动则加选项、已有线性则升级
  为二选、否则设为线性下一集）。无第三方图库（自绘 SVG + 绝对定位，贴合暖白主题）。默认「图」，可切「列表」。
- **整合进短剧工作流**：互动短剧不再是一级导航。在「短剧工坊」(`/projects`) 同列表以「互动剧」标签展示
  + 类型筛选（全部 / 短剧 / 互动剧）。创建路径：① 任意短剧卡片「转互动剧」或项目工作台顶部「转成互动剧」
  （`convertProjectToInteractive`：按分集大纲铺成线性链 → 进编辑器接分支）；② 顶部「AI 起草 / 新建互动剧」。
  旧 `/interactive` 重定向到工坊互动筛选；编辑器「返回」回到短剧工坊。
