...
首页（/dashboard，AI 对话框:短视频/短剧 双类型 + 近期热点 + 创意推荐）
  ...
  │         ① 剧集脚本(剧本+分镜合并;场景卡片 / 脚本表格双视图,[参考N] 引用素材库)
...
| `/shorts/make` | 创作 | **短视频制作（v0.63 新增）**:左 AI 对话 + 右口播脚本表（视频脚本/语音/字幕/参考 四列,[参考N] 内嵌引用）→ 视频工厂双路径出片 → 合成成片 |
| `/templates` | 提效 | **模板库（v0.63 新增）**:多集短剧/单集短视频形态筛选 + 类型筛选 + 统一预览（封面/描述/估时大纲);运营身份可新建模板/爆款拆解入库 |
| `/review` | 提效 | **剧本审阅（v0.63 新增）**:跨项目待审队列 → Excel 式平铺表格（场/场景/动作/角色/对白/情绪/审阅/意见,逐场 通过/改一下,意见发给 AI 重写） |
