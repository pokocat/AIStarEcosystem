...
首页（/dashboard，AI 对话框:短视频/短剧 双类型 + 近期热点 + 创意推荐）
  ...
  └─ 短视频:开做 →  短视频制作（/shorts/make,左 AI 对话 + 右口播脚本表 → 视频工厂）
      ↑ 也可从 提示词直出（/shorts/prompt,粘贴整段提示词 → 拆解预览 → 开拍）进入
      →  短视频工坊（/shorts,短视频 + 宣传片/自传等单集作品资产库;从短剧切片推广）
```

**提效与素材（跨项目复用）**：创意市场（templates，v0.75 原「模板库」：官方内置 + 用户发布统一，源标 官方/来自@xx，套用开拍；子页「我发布的创意」；运营可新建内置/从用户作品精选授权）/ 剧本审阅（review，跨项目待审队列 + Excel 式平铺表格）/ 素材库（assets，标签化图片视频:人物/场景/道具,生成时 @ 作参考）/ 演员 IP 阵容（cast）/ 戏服与道具（wardrobe，建设中）/ 跨项目脚本归档（scripts，建设中）。形象创建（孵化 / 锻造炉）已于 v0.60 收敛至 AiAvatar。
...
| `/projects/new` | 创作 | 新建短剧（选类型 → AI 引导式三步 / 爆款模板式；**v0.79「互动剧」开关** = 立项为剧情互动短剧形态） |
| `/projects/[id]` | 创作 | **短剧工作台（沉浸式,v4 形态）**:项目设置走左阶段轨;剧集制作走左分集导航 + 顶部步骤页签（剧集脚本 → 视频工厂 → 成片配方）。**v0.79 互动剧形态**（`mode=interactive`）：左轨多一个「互动编排」阶段（剧集分支图 + 时间轴互动点 + 全局标记 + 试玩 + 导出 Story Config v2），剧集（图节点）即大纲分集、每集仍走六阶段出片；非互动剧项目左轨可「转换为互动剧」 |
| `/projects/[id]/distribute` | 创作 | 项目多平台发布 |
| `/shorts` | 创作 | **短视频工坊（v0.63 新增）**:短视频 + 宣传片/自传等单集作品资产库;从短剧切片推广入口 |
| `/shorts/prompt` | 创作 | **提示词直出（v0.143 新增）**:粘贴写好的整段提示词 → AI 忠实拆成人物卡 / 场景 / 全片画面基调 / 逐镜分镜 → 预览页逐项修改 → 开拍进工作台。与「一句话生成」并列的第二条入口；拆解免费，开拍扣同一笔 `drama.credit.short-entry` |
| `/shorts/make` | 创作 | **短视频制作（v0.63 新增）**:左 AI 对话 + 右结构化分镜表单（时间线/画面/人声·音效·BGM/镜头参数/特效氛围/参考,@ 引用素材）→ 视频工厂双路径出片 → 合成成片 |
| `/templates` | 提效 | **创意市场（v0.75，原「模板库」）**:官方内置 + 用户发布的可复用创意统一在此（基于已发布 `DramaRecipe`，源标 官方/来自@xx + scope/题材/搜索 + 套用开拍 + 详情）;运营见「新建内置创意 / 从用户作品精选（邀请授权）」 |
| `/templates/published` | 提效 | **我发布的创意（v0.75）**:本人名下创意按状态分档（审核中/已上架/已驳回/运营邀请待授权/已谢绝），invited 行可「授权精选 / 谢绝」 |
