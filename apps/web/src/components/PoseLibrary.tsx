...
import { ArrowLeft, Eye, Hand, Music, Play, Save, Share2, Smile, Sparkles, Wand2 } from "lucide-react";
import type { ExpressionPreset, GesturePreset, PosePreset, SingerDetail } from "@/types/contracts/singers";
...
export function PoseLibrary({ lang, onBack, activeSinger, poses, expressions, gestures }: PoseLibraryProps) {
  const [selectedPose, setSelectedPose] = useState<PosePreset | null>(null);
  const [selectedExpression, setSelectedExpression] = useState<ExpressionPreset | null>(null);
  ...
  const poseCategories = [
    { id: "standing", label: lang === "zh" ? "站姿" : "Standing", count: poses.filter((pose) => pose.category === "standing").length },
    { id: "sitting", label: lang === "zh" ? "坐姿" : "Sitting", count: poses.filter((pose) => pose.category === "sitting").length },
    { id: "dancing", label: lang === "zh" ? "舞蹈" : "Dancing", count: poses.filter((pose) => pose.category === "dancing").length },
    { id: "singing", label: lang === "zh" ? "演唱" : "Singing", count: poses.filter((pose) => pose.category === "singing").length },
    { id: "action", label: lang === "zh" ? "动作" : "Action", count: poses.filter((pose) => pose.category === "action").length }
  ];
            ...
            <div className="lg:col-span-7 space-y-4 overflow-hidden flex flex-col">
              <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                {poseCategories.map((category) => (
                  <Button key={category.id} variant="outline" size="sm" className="flex-shrink-0 border-white/10 hover:bg-white/5">
                    {category.label}
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pb-4">
                  {poses.map((pose, index) => (
                    <motion.div key={pose.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }}>
                          ...
                          <div className="relative aspect-square overflow-hidden">
                            ...
                            <div className="absolute top-2 left-2 right-2 flex justify-between items-start">
                              <div className="flex flex-col gap-1">
                                {pose.isNew && <Badge className="bg-green-500/80 text-white text-xs border-0 font-bold">NEW</Badge>}
            ...
            <div className="lg:col-span-5 bg-[#0c0c0e]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-8 flex flex-col items-center justify-center relative overflow-hidden">
              ...
              {selectedPose ? (
                <>
                  ...
                  <p className="text-cyan-400 text-sm mb-2 capitalize">{selectedPose.category} • {selectedPose.difficulty}</p>
                  <p className="text-gray-500 text-xs mb-6">{activeSinger.name}</p>
                ...
                <CardContent>
                  <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    {expressions.map((expression) => (
                      <motion.div key={expression.id} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={() => setSelectedExpression(expression)} className={`aspect-square rounded-2xl border-2 flex flex-col items-center justify-center cursor-pointer transition-all ${selectedExpression?.id === expression.id ? "bg-pink-500/20 border-pink-500/50 shadow-lg shadow-pink-500/20" : "bg-black/40 border-white/10 hover:border-pink-500/30"}`}>
                        <div className="text-5xl mb-2">{expression.emoji}</div>
                        <div className="text-xs font-bold text-white">{expression.name}</div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  {selectedExpression && (
                    <div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-pink-900/20 to-purple-900/20 border border-pink-500/20">
                      ...
                      <div className="space-y-4">
                        <input type="range" min="0" max="100" value={customIntensity} onChange={(event) => setCustomIntensity(Number.parseInt(event.target.value, 10))} className="w-full accent-pink-500 h-3 bg-gray-800 rounded-lg" />
            ...
            <div className="max-w-5xl mx-auto">
              <Card className="bg-[#0c0c0e]/90 backdrop-blur-xl border-white/10">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-400 mt-2">{lang === "zh" ? "精确控制每个关键节点，创造独特姿态" : "Fine-tune every joint to create unique poses"}</p>
                </CardHeader>
