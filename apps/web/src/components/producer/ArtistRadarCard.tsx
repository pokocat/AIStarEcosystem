...
export const ArtistRadarCard = ({ lang, artist }: { lang: Lang; artist: Artist }) => {
  const zh = lang === 'zh';
  const typeConf = ARTIST_TYPE_CONFIG[artist.type];
  const skills = artist.talents;
  const keys = Object.keys(skills) as (keyof typeof skills)[];
        ...
        <div className="flex items-center gap-3 w-full min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <div className="text-[10px] text-purple-400 font-semibold flex items-center gap-0.5"><Star className="w-3 h-3" /> {maxCap}</div>
            </div>
          </div>
        </div>
