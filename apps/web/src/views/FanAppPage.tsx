...
import type { ChartEntry, DiscoverySpotlight, RecommendationTrack, TrackLyricLine } from "@/types/contracts/tracks";
...
interface FanAppPageProps {
  ...
  marketCollections: NftCollectionSummary[];
...
export function FanAppPage({ lang, copy, chartData, lyrics, marketCollections, discoverySpotlight, recommendations, onBack, onToggleLang }: FanAppPageProps) {
      ...
      <div className="w-full max-w-md bg-black min-h-screen relative shadow-2xl flex flex-col">
        <div className="h-14 px-4 flex items-center justify-between bg-black/80 backdrop-blur-md sticky top-0 z-30 border-b border-white/10">
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={onToggleLang} className="h-8 w-8 text-xs">
              {lang.toUpperCase()}
            </Button>
            <Button variant="ghost" size="icon" onClick={onBack} className="h-8 w-8 rounded-full bg-white/10 hover:bg-white/20">
          ...
          {activeTab === "charts" && (
            <div className="p-4 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary" className="bg-white/10 text-xs">Real-time</Badge>
              </div>
              <div className="flex items-end justify-center gap-4 py-4">
                {[chartData[1], chartData[0], chartData[2]].filter(Boolean).map((item, index) => {
                    ...
                    <div key={item.id} className={`flex flex-col items-center gap-2 w-1/3 ${rank === 1 ? "-mt-6" : ""}`}>
                      <div className="relative">
                        ...
                        <div className={`${size} rounded-full border-2 ${border} p-1 shadow-[0_0_20px_rgba(250,204,21,0.3)]`}>
                          <img src={item.coverUrl} className="w-full h-full rounded-full object-cover" alt={item.title} />
                        </div>
                  ...
                  <div key={item.id} className="flex items-center gap-4 bg-white/5 p-3 rounded-xl border border-white/5 hover:bg-white/10 transition-colors">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className="text-xs font-bold text-gray-300 flex items-center gap-1">
                        <Flame className="w-3 h-3 text-gray-500" /> {item.votes}
                      </div>
                      <Button size="icon" variant="ghost" className="h-6 w-6 rounded-full border border-white/10 hover:bg-pink-500/20 hover:text-pink-500">
          ...
          {activeTab === "market" && (
            <div className="p-4 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {marketCollections.map((item) => (
                  <div key={item.id} className="bg-[#121214] border border-white/10 overflow-hidden rounded-xl">
                    <div className="aspect-square bg-gray-800 relative">
                      <img src={item.coverUrl} className="w-full h-full object-cover" alt={item.name} />
                      <div className="absolute top-2 right-2 bg-black/60 backdrop-blur px-2 py-1 rounded text-[10px] text-white border border-white/20">
                        {copy.remaining} {item.remaining}
                      </div>
                    </div>
                    <div className="p-3">
          ...
          {activeTab === "discovery" && (
            <div className="p-4 space-y-6">
              <div className="relative h-48 rounded-2xl overflow-hidden">
                <img src={discoverySpotlight.coverUrl} className="w-full h-full object-cover" alt={discoverySpotlight.title} />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
                <div className="absolute bottom-4 left-4">
                  <Badge className="bg-pink-600 border-0 mb-2">{discoverySpotlight.badge}</Badge>
                  <h2 className="text-2xl font-bold">{discoverySpotlight.title}</h2>
                  <p className="text-sm text-gray-300">{discoverySpotlight.subtitle}</p>
                </div>
              </div>
              <div>
                <h3 className="font-bold mb-4">{copy.guess_like}</h3>
          ...
          <div onClick={() => setShowFullPlayer(true)} className="absolute bottom-16 left-4 right-4 bg-[#1c1c1e]/90 backdrop-blur-md border border-white/10 rounded-full p-2 pr-4 flex items-center justify-between shadow-xl z-20 cursor-pointer hover:scale-105 transition-transform">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full bg-gray-800 overflow-hidden border border-white/10 ${isPlaying ? "animate-spin-slow" : ""}`}>
                <img src={topSong.coverUrl} className="w-full h-full object-cover" alt={topSong.title} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <div className="h-4 flex items-center gap-0.5">
                {[...Array(6)].map((_, index) => (
                  <div key={index} className={`w-0.5 bg-cyan-500 rounded-full ${isPlaying ? "animate-music-bar" : "h-1"}`} style={{ animationDelay: `${index * 0.1}s` }} />
                ))}
              </div>
              <div onClick={(event) => { event.stopPropagation(); setIsPlaying((current) => !current); }} className="w-8 h-8 rounded-full bg-white text-black flex items-center justify-center hover:bg-gray-200">
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-8">
                ...
                <div className="w-full space-y-4">
                  <div className="h-32 overflow-hidden relative mask-linear-y text-center">
                    <div className="space-y-4">
                      {lyrics.map((line, index) => (
                        <p key={`${line.time}-${index}`} className={`text-lg transition-all duration-500 ${index === 1 ? "text-white font-bold scale-110" : "text-gray-600"}`}>
                          {line.text}
                        </p>
                      ))}
                    </div>
                  ...
                  <div className="flex justify-between items-center px-4">
                    ...
                    <div onClick={() => setIsPlaying((current) => !current)} className="w-16 h-16 rounded-full bg-white text-black flex items-center justify-center hover:scale-105 transition-transform cursor-pointer shadow-lg shadow-white/20">
