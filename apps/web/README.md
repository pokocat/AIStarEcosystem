...
自 v2 起，前端以 `src/types/*` 为数据契约的唯一真值源。`apps/web/src/api/*.ts` 是调用契约（`USE_MOCK` 切换 mock vs 真后端，但二者返回都按 TS 接口形状）。后端接口契约由 [`specs/openapi.yaml`](../../specs/openapi.yaml) 定义；openapi 表达不了的业务规则（校验、扣费公式、错误码、状态机时序）放在 [`specs/BUSINESS_RULES.md`](../../specs/BUSINESS_RULES.md)。CI 漂移校验由 `npm run check:api-contract` 守护——每条 `apiFetch(...)` URL 必须在 openapi.yaml 有对应 path。
...
| 文件 | 角色 |
|---|---|
| **`apps/web/src/types/*.ts`** | **唯一前端真值源**（CLAUDE.md 第一条硬规则）。后端 Spring `*Dto` 字段名必须与之逐字一致。|
| `apps/web/src/api/*.ts` | 调用契约。`apiFetch<T>(...)` 的泛型即返回类型；`USE_MOCK` 切换不改变契约。|
| [`specs/openapi.yaml`](../../specs/openapi.yaml) | 后端接口契约。142 个 path、200+ schema。|
| [`specs/BUSINESS_RULES.md`](../../specs/BUSINESS_RULES.md) | openapi 表达不了的业务约束：校验规则、扣费公式、错误码、状态机时序。|
| `apps/web/scripts/check-api-contract.mjs` | CI 漂移校验：`apiFetch` URLs ↔ openapi paths。|

新增领域 / 接口的顺序：

1. `apps/web/src/types/<domain>.ts` 定义 TS 接口 → 前端真值源。
2. `apps/web/src/api/<domain>.ts` 写 `apiFetch<T>('/your/path')`。
3. `specs/openapi.yaml` 同步路径 + schema。
4. `npm run check:api-contract` 验证（CI 也会跑）。

> 旧版 `specs/FRONTEND_CONTRACT_DIFF.md` 在 v2.7 已删除——drift 由 CI 校验取代「过期的 diff 文档」。
