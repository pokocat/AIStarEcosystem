...
import { AssetPortrait, Badge, Card, EmptyState, HubScreen, ListRow, LoadingBlock, NavBar, RegNo, SectionHeader } from "@/components/hub/ui";
...
function bigName(c: Avatar): string {
  ...
  const base = latin || (c.codename || "").replace(/-/g, " ");
  return (base || "ASSET").toUpperCase().slice(0, 12);
}
...
function gaps(c: Avatar): { done: number; total: number; items: { text: string; href: string }[] } {
  ...
  else items.push({ text: `补齐标准图：还差 ${DATA.SHOTS.length - shotDone} 张，出图时脸更稳定`, href: studioHref(`#/avatar/${c.id}/looks`) });
  const derivDone = DATA.DERIVS.filter((d) => c.deriv?.[d.key] === "done").length;
  if (derivDone >= DATA.DERIVS.length) done += 1;
  else items.push({ text: `补一套服装：换场景时衣服不会变来变去`, href: studioHref(`#/avatar/${c.id}/ward`) });
  if (c.voiceName) done += 1;
  else items.push({ text: "绑一个声音：视频里就是她自己的声线", href: studioHref(`#/avatar/${c.id}/voice`) });
  if (c.def?.设定语) done += 1;
  else items.push({ text: "写两句人设：AI 写台词时知道她该怎么说话", href: studioHref(`#/avatar/${c.id}`) });
...
export default function AssetCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const groups = useMemo(
    () =>
      [
        ...
        { key: "shot", label: "视角", items: shotItems },
  ...
  const hero: Hero = useMemo(() => {
    const all = [...(byKey.video || []), ...(byKey.expr || []), ...(byKey.ward || []), ...shotItems];
    ...
    if (src?.fileUrl && VIDEO_RE.test(src.fileUrl)) {
  ...
  return (
    <HubScreen>
      ...
      <div style={{ position: "relative", height: 524, background: "linear-gradient(178deg, #D6E3EC 0%, #DEE9F0 46%, var(--canvas) 100%)", overflow: "hidden", flexShrink: 0 }}>
        ...
        <div style={{ position: "absolute", top: 104, left: "50%", transform: "translateX(-50%)", width: 232, height: 278, borderRadius: 18, overflow: "hidden", zIndex: 2, boxShadow: "0 24px 48px rgba(24,44,62,.22)", background: "var(--surface-3)" }}>
          {hero.kind === "video" ? (
            ...
            <video src={hero.url || undefined} poster={hero.poster || undefined} autoPlay loop muted playsInline style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          ) : hero.kind === "image" ? (
        ...
        <div style={{ position: "absolute", left: 0, right: 0, bottom: 16, display: "flex", flexDirection: "column", alignItems: "center", gap: 7, zIndex: 3, padding: "0 20px" }}>
          ...
          <span style={{ fontFamily: "var(--font-serif)", fontSize: 30, fontWeight: 600, lineHeight: 1 }}>{c.name}</span>
          {c.def?.设定语 && (
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        {c.voiceName && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, padding: "11px 13px", background: "var(--primary-tint)", borderRadius: 14 }}>
            <span style={{ width: 30, height: 30, borderRadius: 999, background: "var(--primary)", display: "grid", placeItems: "center", flexShrink: 0 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="#fff" stroke="none"><polygon points="8 5 19 12 8 19 8 5" /></svg>
            </span>
            <div style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 2.5, height: 22 }}>
              {[8, 15, 22, 11, 19, 7, 17, 24, 12, 9, 20, 14, 8, 18, 10, 21, 13].map((h, i) => (
                <span key={i} style={{ width: 2.5, height: h, borderRadius: 9, background: h > 18 ? "var(--primary)" : h > 11 ? "#6BC6E2" : "#9FD8EA" }} />
              ))}
            </div>
            <span className="mono" style={{ fontSize: 10, color: "var(--ink-2)", flexShrink: 0, maxWidth: 96, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.voiceName}</span>
          </div>
        )}
      </div>
      ...
      {/* ── 被用在哪 ───────────────────────────────── */}
      <div style={{ margin: "18px 16px 0" }}>
        <SectionHeader title="她出过的片" count={usageKnown ? usedIn.length : undefined} />
        {!usageKnown ? (
