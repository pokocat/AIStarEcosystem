...
function LoginInner() {
  ...
  const doDevLogin = async () => {
    ...
    try {
      await loginAs(username || undefined);
      await refresh();
      router.replace(from);
    } catch (e) {
  ...
  const sendCode = async () => {
    ...
    setBusy(true); setErr(null);
    try {
      await AuthApi.smsRequestCode(phone, "login");
  ...
  const doPhoneLogin = async () => {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : "登录失败，请确认验证码");
    } finally { setBusy(false); }
  };

  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--bg-0)] p-6 text-[var(--fg-0)]">
      <div className="w-full max-w-sm rounded-2xl border border-zinc-800 bg-[var(--bg-1)] p-7 shadow-2xl">
        <div className="mb-6 flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            <div className="text-sm font-semibold">AiAvatar 中心</div>
            <div className="font-mono text-[10px] text-zinc-500">登录以管理你的AiAvatar 资产</div>
          </div>
        </div>
        ...
        <div className="mb-5 flex gap-1 rounded-lg bg-zinc-800 p-1">
          {ENABLE_DEV_LOGIN && (
            <button onClick={() => setTab("dev")}
              className={`flex-1 rounded-md py-1.5 text-xs font-medium ${tab === "dev" ? "bg-amber-500 text-zinc-950" : "text-zinc-400"}`}>
              开发登录
            </button>
          )}
          <button onClick={() => setTab("phone")}
            className={`flex-1 rounded-md py-1.5 text-xs font-medium ${tab === "phone" ? "bg-amber-500 text-zinc-950" : "text-zinc-400"}`}>
            手机号登录
          </button>
        </div>
        ...
        {tab === "dev" && ENABLE_DEV_LOGIN ? (
          <div className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-xs text-zinc-400">选择种子账号</span>
            ...
            <button onClick={doDevLogin} disabled={busy}
          ...
          <div className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-xs text-zinc-400">手机号</span>
            ...
            <label className="block">
              <span className="mb-1 block text-xs text-zinc-400">验证码</span>
              <div className="flex gap-2">
                ...
                <button onClick={sendCode} disabled={busy || countdown > 0}
                  className="shrink-0 rounded-lg border border-zinc-700 px-3 text-xs text-zinc-300 hover:border-zinc-500 disabled:opacity-50">
                  {countdown > 0 ? `${countdown}s` : sent ? "重发" : "发送"}
                </button>
              </div>
            </label>
            <button onClick={doPhoneLogin} disabled={busy || !code}
              className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-amber-500 py-2.5 text-sm font-semibold text-zinc-950 hover:bg-amber-400 disabled:opacity-60">
        ...
        {err && <p className="mt-3 rounded-lg bg-rose-500/10 px-3 py-2 text-xs text-rose-400">{err}</p>}
      </div>
    </div>
  );
}
