...
| 真人捕获 | 创建 sheet ／ 各处「去确认」入口 | 录制引导 → 倒计时录制 → 上传素材 → **确认新版授权协议 → 前往七牛云真人核验（同页打开）→ 自动回到当前授权会话 → 服务端确认活体与同人结果 → 登记授权** → 复刻生成 → 选音色 → 保存。短链接失效时重新发起会话；刷新、登录后或手动返回均可恢复进度。已出形象的资产走「补确认」，完成后不重复复刻 |
