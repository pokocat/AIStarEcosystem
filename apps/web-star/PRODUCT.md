...
| /profile | 档案设置 ★ | v0.62：明星市场展示档案自维护（编辑权从 admin 移入）；头像/封面上传 + 营销字段；运营字段（isHot/套餐/配额）不开放 |
...
- **可访问性**：全局 `:focus-visible` 焦点环；`prefers-reduced-motion` 停用动画；
  状态不只靠颜色（图标 + 文案并行）。
- **响应式**：≥1024 侧导航 240px；640-1023（平板）顶部横向 Tab（活跃模块自动滚动居中 +
  右端「全部」按钮打开分组模块抽屉）；**<640（手机）底部 Tab 栏**（主流移动导航模式，
  顶部不再横滑）—— 固定 5 槽：总览 / 带货授权 / 内容审核 / 商品入库 / 全部
  （前三为两条 ★ 打通链路 + 审批阀门；「全部」打开分组模块抽屉，其余模块待办计入其
  红点角标；身处非 Tab 模块时「全部」高亮示位）。槽位真值：
  `constants/star-ui.ts` 的 `STAR_BOTTOM_TABS`。
...
- **移动端禁横滑原则（<sm）**：不引入横向滚动（导航为底部 Tab 栏）—— KPI 卡竖排（标签行→
