...
3. 结论写入 `apps/web_new/specs/FRONTEND_CONTRACT_DIFF.md`，并在摘要段给出优先级。
...
5. 再跑一次 P3（契约 diff）验证前端结构变化是否影响后端；差异更新到 `FRONTEND_CONTRACT_DIFF.md`。
6. README 记录一条重置条目。
...
| 新增枚举值 | 先改 `types/`，再补 `constants/` 颜色/标签，最后改组件；同步 `FRONTEND_CONTRACT_DIFF.md` |
...
- `apps/web_new/specs/FRONTEND_CONTRACT_DIFF.md` — 与后端 `specs/openapi.yaml` 的差异清单
- `specs/openapi.yaml` — 后端 API 契约（前端发现缺口时驱动后端补齐）
