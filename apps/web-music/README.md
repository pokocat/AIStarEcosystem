...
### v0.138 · 2026-08-29 · 音乐创作接入真实大模型 {#v0138}

此前整条创作链路是假的：前端用 `setTimeout` 播放写死文案伪装流式、从 4 条预写草稿里随机抽一条当结果；后端只存一行记录，音频是硬编码占位符，界面显示的"消耗积分"是随机数且从不真扣。现在：

- **真实生成**：`POST /me/music/generate` → 异步任务 → 轮询 → 成品音频镜像到我方存储。接火山「AI 音乐生成大模型」（`doubao-music`）。
- **三种模式**：灵感成曲（描述→AI 写词谱曲演唱）/ 我有歌词（支持 `[verse]`/`[chorus]` 结构标签）/ 纯音乐（无人声，≤60 秒）。
...
### v0.137 · 2026-08-29 · 创作工坊去掉艺人硬闸（自由创作） {#v0137}
