...
const COVERAGE_OPTIONS: { value: StickerPoolBinding["coverage"]; label: string; hint: string }[] = [
  { value: "intro", label: "开头 3s", hint: "片头吸睛装饰" },
  { value: "outro", label: "结尾 3s", hint: "片尾收束氛围" },
  { value: "loop", label: "整片", hint: "持续装饰整段" },
  { value: "random_3s", label: "随机 3s", hint: "每变体随机时段" },
];
...
export function StickerPoolPicker({ value, onChange, disabled, label = "扰动贴图池" }: Props) {
  ...
  return (
    <div className="rounded-lg border border-border bg-secondary/30 p-3 space-y-3">
        ...
        <div className="flex items-center gap-1.5 text-sm font-medium">
          ...
          {selectedIds.length > 0 && (
            <span className="text-xs text-violet-500 font-normal">
              已选 {selectedIds.length} 张 · 每变体随机抽 {value?.pick_count ?? 1} 张
            </span>
          )}
        </div>
      ...
      {loading && (
        <div className="text-xs text-muted-foreground py-4 text-center">加载预置贴图池中…</div>
      )}
      {error && (
        <div className="text-xs text-rose-500 py-4 text-center">加载失败：{error}</div>
      )}
      {!loading && !error && groups.length === 0 && (
        <div className="text-xs text-muted-foreground py-4 text-center">
          暂无预置扰动贴图。请联系管理员上传到 /api/admin/mixcut/preset-stickers。
        </div>
      )}
      ...
      {value && (
        <div className="space-y-3 pt-3 border-t border-border/60">
          ...
          <div className="text-[11px] text-muted-foreground leading-tight">
            提示：GIF 二值透明度限制，半透明效果通过整体调薄实现。每变体按 (jobId + variantIndex)
            随机抽样，可复现。
          </div>
        </div>
      )}
