...
const FEATURES: Array<{ icon: React.ReactNode; title: string; desc: string }> = [
  ...
  {
    icon: <Pencil className="h-4 w-4 text-violet-300" />,
    title: "AI 写脚本",
  ...
  {
    icon: <RefreshCcw className="h-4 w-4 text-violet-300" />,
    title: "不满意可重抽",
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-xl border border-violet-500/25 bg-gradient-to-br from-violet-500/[0.08] to-pink-500/[0.04] p-4">
        ...
        <div className="rounded-xl border border-dashed border-zinc-200 bg-zinc-50 p-4">
          <div className="mb-2 text-sm font-medium text-zinc-600">创意倾向（选填）</div>
        ...
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-lg border-2 border-violet-400/50 bg-gradient-to-r from-violet-500/40 via-violet-500/30 to-pink-500/30 px-5 py-3 text-base font-semibold text-zinc-900 shadow-[0_0_30px_rgba(168,85,247,0.35)] transition hover:shadow-[0_0_40px_rgba(168,85,247,0.55)] disabled:cursor-not-allowed disabled:border-zinc-200 disabled:from-white/10 disabled:via-white/10 disabled:to-white/10 disabled:text-zinc-300 disabled:shadow-none"
        >
          <Dice5 className="h-4 w-4" /> 开盲盒生成
        </button>
        <div className="flex flex-col gap-1.5 text-[11px]">
          <div className="text-center text-zinc-400 tabular-nums">
            消耗 <span className="text-violet-200">✦{formatCredits(creditPrice)}</span> 积分
            <span className="text-zinc-300"> · 占套餐 {cost} 条额度 · 预计 {meta.speed}</span>
          </div>
          {insufficientCredits && (
            <Link
          ...
          {!insufficientCredits && insufficientQuota && (
            <span className="self-center text-pink-300">⚠ 套餐额度不足，将使用积分扣费</span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
          <div className="mb-3 text-sm font-medium text-zinc-600">盲盒模式说明</div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {FEATURES.map((f) => (
              <div
                key={f.title}
                className="flex gap-2.5 rounded-lg border border-zinc-100 bg-zinc-50 p-3"
              >
                <div className="mt-0.5">{f.icon}</div>
                <div>
                  <div className="text-sm font-semibold text-zinc-700">{f.title}</div>
                  <p className="mt-0.5 text-[11px] leading-relaxed text-zinc-400">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
          <div className="mb-3 text-sm font-medium text-zinc-600">盲盒往期作品</div>
            ...
            {showcases.map((s) => (
              <div key={s.id}>
                <CelebrityVideoPlayer
                  ...
                  aspect="9/16"
                />
                <p className="mt-1.5 text-[11px] text-zinc-400">{s.caption}</p>
                <p className="text-[10px] text-zinc-300">{s.approval}</p>
              </div>
            ))}
          </div>
        </div>
