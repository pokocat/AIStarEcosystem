        ...
        {STATUS_TABS.map((s) => (
          <button
            ...
            className={cn(
              ...
              status === s ? "text-cyan-300" : "text-white/45 hover:text-white/80",
            )}
          >
            {s}
            <span className="ml-1 text-[10px] text-white/30">({counters[s] ?? 0})</span>
            {status === s && (
...
function FilterSelect<T extends string>({
  ...
  return (
    <label className="inline-flex items-center gap-2 rounded-md border border-white/10 bg-white/[0.02] px-2 py-1 text-xs text-white/55">
