...
// 明星新增表单（v0.55+：web-celebrity 内嵌运营管理；v0.62 起编辑模式下线 ——
// 档案编辑移交明星商务工作台 web-star /profile，此处仅保留运营新增）。
// 写操作走 CelebrityZoneApi.createStar（URL → /admin/celebrity/stars）；
...
interface Props {
  // v0.62：编辑模式已下线（档案编辑移交明星商务工作台 web-star /profile），仅保留新建。
  onClose: () => void;
  onSaved: () => void | Promise<void>;
}

export function StarFormDialog({ onClose, onSaved }: Props) {
  const [name, setName] = React.useState("");
  const [category, setCategory] = React.useState<CelebrityCategory>("演员");
  const [avatar, setAvatar] = React.useState("");
  const [cover, setCover] = React.useState("");
  const [description, setDescription] = React.useState("");
  const [pricingTier, setPricingTier] = React.useState<PricingTierName>("标准版");
