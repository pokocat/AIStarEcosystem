...
- **资金财务**（v2 §6：FINANCE_ADMIN 专属，OPERATOR 看不到/后端 403；新增 `/finance` 控制台首页）：**充值订单**（核准入账 / 驳回 / 退款）、对账、结算中心、异常风控、**充值套餐**
...
- **`/platform/ai-models`**（v0.5 起；v0.41 合并「外部 API Token」）— 双 Tab：模型接入端点（上游密钥+单模型+地址，含外部 Token）/ AI 应用绑定
...
- **v0.57 / 2026-06-09**：`/platform/auth-logs`（账号登录日志）新增「来源应用」列 + 筛选下拉，区分登录来自哪个子应用（music / drama / celebrity / aiavatar / star / celebrity-mp 小程序 / admin）。数据来自各前端注入的 `X-App-Code` 请求头 → server `AuditLog.appCode` 列。`types/audit.ts` 增 `appCode` + `APP_CODE_LABEL/KEYS`；`api/audit.ts` 两入参增 `appCode`；admin 自身 `api/_client.ts` 注入 `X-App-Code=admin`。老数据无来源 → 显示 "—"。详见 AGENTS.md / VERSION_HISTORY.md v0.57。
...
- **v0.41 / 2026-05-29**：合并「AI 模型」+「外部 API Token」两个菜单为一个「AI 模型与 Token」（`/platform/ai-models`，删 `/platform/llm-keys`）。改为三 Tab：①**模型接入端点（含 Token）**—— 固定 {上游密钥 + 单模型 + 地址} 的 CRUD + 测试连接 + 获取模型列表选固定模型 + 生成/撤销外部 Token（`sk-aep-*`，明文一次横幅）+ 计费归属用户（空=平台级不计费）；②**AI 应用绑定** —— 7 个用途各一个端点下拉（脚本起草 / 卖点提取 / 变量抽取等），一用途一端点、无兜底；③**用量统计** —— 时间窗下拉（近 1/7/30/90/365 天）+ 4 个汇总数 + 按端点 / 模型 / 用途 / 用户 / 租户 / 来源应用维度，数据来自 server 自建 token 流水聚合（`GET /api/admin/ai-models/usage`，不依赖各家计费接口）。对应 server `AdminAiModelEndpointController`（+`/{id}/mint-key`、`/{id}/revoke-key`、`/usage`）+ `AdminAiAppBindingController`（`/api/admin/ai-app-bindings`）。`api/ai-models.ts` +`getUsage`/`getProviderUsage`/绑定函数；`api/llm-keys.ts` + `LlmKeysApi` 删除。
