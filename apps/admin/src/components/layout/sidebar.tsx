    ...
    <aside className="sticky top-0 hidden h-screen w-72 shrink-0 border-r border-sidebar-border bg-sidebar lg:flex lg:flex-col">
      <div className="border-b border-sidebar-border px-5 py-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-semibold tracking-[0.24em] text-sidebar-foreground/45">
                AI Star Eco
              </p>
              <p className="mt-1 text-base font-semibold text-sidebar-foreground">账户与权益后台</p>
            </div>
          </div>
      ...
      <div className="border-t border-sidebar-border px-4 py-4">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3">
          <p className="text-xs font-medium text-sidebar-foreground/55">当前环境</p>
          ...
          <p className="mt-1 text-xs leading-5 text-sidebar-foreground/50">
            默认对接 `http://localhost:8080`，适合联调账户、卡密和积分接口。
          </p>
        </div>
      </div>
    </aside>
