 ...
 */
public class V20__song_owner_and_optional_artist extends BaseJavaMigration {

    private static final Logger log = LoggerFactory.getLogger(V20__song_owner_and_optional_artist.class);
    ...
    public void migrate(Context context) throws Exception {
        try (Statement st = context.getConnection().createStatement()) {
            try {
                ...
                log.info("[V20-song-owner] added aep_songs.owner_user_id");
            } catch (Exception e) {
                log.debug("[V20-song-owner] skip owner_user_id: {}", e.getMessage());
            }
            try {
                // MySQL 语法；失败再试 H2 语法（已有 H2 文件库升级场景）。
                st.executeUpdate("ALTER TABLE aep_songs MODIFY COLUMN artist_id VARCHAR(36) NULL");
                log.info("[V20-song-owner] relaxed aep_songs.artist_id NOT NULL (mysql)");
            } catch (Exception mysqlEx) {
                try {
                    st.executeUpdate("ALTER TABLE aep_songs ALTER COLUMN artist_id SET NULL");
                    log.info("[V20-song-owner] relaxed aep_songs.artist_id NOT NULL (h2)");
                } catch (Exception h2Ex) {
                    log.debug("[V20-song-owner] skip artist_id relax: {}", h2Ex.getMessage());
                }
            }
            try {
                st.executeUpdate("CREATE INDEX idx_songs_owner_user ON aep_songs (owner_user_id)");
                log.info("[V20-song-owner] added idx_songs_owner_user");
            } catch (Exception e) {
                log.debug("[V20-song-owner] skip idx_songs_owner_user: {}", e.getMessage());
            }
        }
    }
}
