 ...
 * byUser / byTenant / byAppCode 为归属维度聚合；仅统计成功调用。
 * byDay 为按自然日的时间序列。failedCalls 为该窗内失败调用数（单独计数）。
...
public record AiModelUsageReportDto(
        ...
        List<AiModelUsageStatDto> byUser,
        List<AiModelUsageStatDto> byTenant,
        List<AiModelUsageStatDto> byAppCode,
