 ...
 * 它不代表任何授权关系。明星已被软删的授权行同样不投影（不把已下架明星
 * 渲染成可用授权）。排序：生效中 → 审批中 → 已到期，同组内按更新时间倒序。
 */
@Service
public class DapStarGrantService {
    ...
    public List<DapStarGrantDto> list(String userId) {
        ...
        Map<String, CelebrityStar> starById = stars.findByIdInAndDeletedAtIsNull(
        ...
        return rows.stream()
                .filter(a -> starById.containsKey(a.getStarId()))
                .sorted(Comparator
