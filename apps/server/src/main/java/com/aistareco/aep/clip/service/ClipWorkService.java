...
public class ClipWorkService {
    ...
    private final ClipProjectRepository projects;private final ClipRenderJobRepository jobs;private final FileStorageService storage;private final ShiliuService shiliu;
    public ClipWorkService(ClipProjectRepository projects,ClipRenderJobRepository jobs,FileStorageService storage,ShiliuService shiliu){this.projects=projects;this.jobs=jobs;this.storage=storage;this.shiliu=shiliu;}
    ...
    private WorkDto dto(ClipProject p){ClipRenderJob j=jobs.findFirstByProjectIdAndExternalOwnerIdOrderByCreatedAtDesc(p.getId(),p.getExternalOwnerId()).orElse(null);List<Map<String,String>> stats=publishStats(p.getPayloadJson());String status=!stats.isEmpty()?"published":"generating".equals(p.getStatus())?"generating":"done";return new WorkDto(p.getId(),p.getId(),p.getTitle(),status,p.getDurationSec(),p.getAvatarSeconds(),p.getCreditsHeld(),j==null?null:storage.signedUrl(j.getOutputCdnKey()),j==null?null:storage.signedUrl(j.getThumbnailCdnKey()),stats);}
