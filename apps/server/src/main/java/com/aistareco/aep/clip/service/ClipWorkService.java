...
public class ClipWorkService {
    ...
    private WorkDto dto(ClipProject p){ClipRenderJob j=jobs.findFirstByProjectIdAndExternalOwnerIdOrderByCreatedAtDesc(p.getId(),p.getExternalOwnerId()).orElse(null);if(j!=null&&j.getThumbnailCdnKey()==null&&j.getOutputCdnKey()!=null){try{var thumb=thumbnails.extract(p.getExternalOwnerId(),j.getOutputCdnKey());j.setThumbnailCdnKey(thumb.key());jobs.save(j);}catch(Exception ignored){}}List<Map<String,String>> stats=publishStats(p.getPayloadJson());String status=!stats.isEmpty()?"published":"generating".equals(p.getStatus())?"generating":"done";return new WorkDto(p.getId(),p.getId(),p.getTitle(),status,p.getDurationSec(),p.getAvatarSeconds(),p.getCreditsHeld(),j==null?null:storage.signedUrl(j.getOutputCdnKey()),j==null?null:storage.signedUrl(j.getThumbnailCdnKey()),stats);}
