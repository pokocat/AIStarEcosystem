...
ALTER TABLE clip_template
    ADD COLUMN source_project_id VARCHAR(64) NULL COMMENT '存成这套模板的那条草稿；NULL=不是从草稿存的',
    ADD COLUMN created_by VARCHAR(128) NULL COMMENT '执行存模板的运营（externalOwnerId）；NULL=本列上线前建的';
