...
- **v0.42（2026-05-29）**：素材运营「带货视频生成」接真后端（异步 submit + 轮询）。新实体 `MaterialVideoJob`（`material_video_job` 表）+ `MaterialVideoModelClient`（视频大模型「提交+轮询」HTTP 客户端，单一可替换点；provider 取自后台「AI 模型」用途 `VIDEO_GENERATION`，submit/poll 协议走 `aep.material.video.*`，默认对齐 智谱 CogVideoX 异步约定；未配 → `VIDEO_NOT_CONFIGURED` 503）+ `MaterialVideoJobService`（提交扣费+派发 / 查询 / wire 映射）+ `MaterialVideoWorker`（`@Async("materialVideoExecutor")` 服务端轮询直到出片/超时，成功 `commitHold` / 失败 `releaseHold`）。`AiModelPurpose` +`VIDEO_GENERATION`；`CelebrityActionPricingService` +action `material.video-generate`（默认 30/条）。`MaterialOpsController` +`/material/videos/generate` + `/material/videos/jobs[/{id}]`。配套修脚本预览关联商品错配（前端按 `product_id` 查全量商品库）+ 基线生成直给。详见根目录 [`AGENTS.md`](../../AGENTS.md) §v0.42。
- **v0.41（2026-05-29）**：大模型用量统计（自建 token 流水）。新增 `ai_model_usage_record` 表（`AiModelUsageRecord`：providerId / providerName / model / purpose / promptTokens / completionTokens / totalTokens / success / createdAt）+ `AiModelUsageRecordRepository`（Object[] 聚合查询）+ `AiModelUsageService`（`record(...)` best-effort 落库 + `report(days)` / `reportForProvider(id, days)` 聚合，days 缺省 30 封顶 365）。`AiModelInvocationService.doChat` 解析 `prompt_tokens`/`completion_tokens`（之前只取 `total_tokens`）并在末尾落流水。新增 `GET /api/admin/ai-models/usage` + `GET /api/admin/ai-models/{id}/usage`。各厂商无统一用量协议，故不打各家计费接口，而是把每次响应里已返回的 `usage` 自行落库聚合（对所有 OpenAI 兼容 provider 通用）。仅记成功调用；record 走 `REQUIRES_NEW` 独立事务 + try/catch，绝不阻断 chat。
...
| `ai_model_usage_record` | 大模型调用用量流水（v0.41；每次成功 chat 落一行，记 provider/model/purpose + prompt/completion/total tokens；只追加，供 admin 用量统计聚合） |
