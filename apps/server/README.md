...
- **v0.45（2026-05-31）**：web-drama 前后端数据对齐补齐 + 短剧「完整创作工作流」。
  - **脚本工坊**：新实体 `WorkshopScript`（`workshop_scripts`）+ `WorkshopScriptVersion`（`workshop_script_versions`）+ `ScriptWorkshopService`（CRUD + 版本树 + AI 续写，复用 `AiModelInvocationService` 用途 `DRAMA_SCRIPT_DRAFT`）+ `ScriptWorkshopController`（`/api/me/scripts/**` + `/api/me/script-versions/{id}`）；`ScriptDto`/`ScriptVersionDto` 镜像 `packages/types/src/script.ts`。
  - **提现**：`CreditService.withdraw`（仅扣 recharge 桶，写 `WITHDRAW` LedgerEntry，余额不足 402）+ `WalletWithdrawController`（`POST /api/me/wallet/withdraw`）+ `WithdrawalRequestDto`。
  - **短剧项目 CRUD**：`FilmController` 扩为可写（`/api/film/dramas` POST + `/{id}` GET/PATCH/DELETE + `/{id}/status` PATCH；复用 `Drama`/`DramaDto`）。
  - **短剧发布任务**：新实体 `DramaPublishJob`（`drama_publish_jobs`）+ `DramaPublishService`（读时推进进度模拟 queued→…→live）+ `DramaDistributionController`（`/api/distribution/jobs/**`）；DTO 镜像 `PublishJob` 消费子集。**不接 sau**（drama 无社媒绑定）。
  - **短剧工作流增强**：`DramaScript` +3 列 `series_id`/`episode_no`(`@ColumnDefault("0")`)/`drama_id`；`payloadJson` 加性扩展（scenes +景别/运镜/配音开关/出场角色，顶层 +characters/style/variants）。`DramaScriptService` 注入只读 `DigitalIpRepository`（演员形象合成）+ `DramaRepository`（成片归入项目）；`buildEpisodePrompt` 增角色设定 + 分镜元数据；新方法 `rewriteScene`/`listSeriesEpisodes`/`publishToProject`；`generateEpisodes` 支持变体。`DramaController` +`/scenes/rewrite`、`/series/{seriesId}/episodes`、`/scripts/{id}/publish-to-project`。
  - **演示数据**：`DramaDemoSeeder`（`@Profile dev/test`、`@Order 101`）幂等种 drama/movie/ad/voice-work/fan-tier/fan-activity/community-event/fan-growth/distribution-content + 2 条演示短剧脚本（补齐 `DemoCatalogSeeder` 跳过的领域）。
  - JPA 加列均 nullable/`@ColumnDefault`，`ddl-auto=update` 自动建表/加列。⚠️ `/api/film/**`、`/api/distribution/**` 仍 permitAll（生产硬化项）。详见根目录 [`AGENTS.md`](../../AGENTS.md) §v0.45。
