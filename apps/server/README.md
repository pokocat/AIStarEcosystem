...
- **v0.85（2026-06-18）**：短剧视觉一致性中间件（移植 [ViMax](https://github.com/HKUDS/ViMax)「参考注入」思路，**不引入其依赖**——提示词进 `PromptService`、选择走既有 Java chat）。① **角色定妆三视图**：`DramaRenderService.renderPortraits` + 端点 `POST /api/me/drama/render/portraits`（`drama.character_portrait` 提示词，正/侧/背三图经 `callImageModel`→`uploadPng` 落 CDN，单价 `drama.credit.portrait` 默认 6，`/me/drama/config` 下发）。未配图像模型 → 503 `IMAGE_NOT_CONFIGURED`。② **出图前一致性选择**：`renderFrame` 加可降级旁路 —— body 传 `ref_pool([{url,desc}])`/`frame_desc`/`consistency` 即调 `selectReferences`（`invokeChat(DRAMA_SCRIPT_DRAFT,…)` 文本 chat + `drama.ref_select` 提示词，挑 ≤6 张参考 + 改写参考说明 → 拼进 prompt + 作 `ref_images` 注入），响应加 `consistency:{used,clause,selected,reason}`；因 `DramaFrameJobWorker` 回放 body 进 `renderFrame`，async frame-job 零改动同样生效。`PromptService` 新 key `drama.ref_select` / `drama.character_portrait`（并入 `KNOWN_KEYS` 自动 seed）；`Material` 加 `url`/`cdnKey`（场景参考图存素材库）。**§8.0**：文本端点未绑 / prompt origin=code / 调用 / 解析失败 → `SelectionResult.degraded` 退回原始出图（真实产物 `used=false`，**不伪造、不阻断、不额外扣费**；图像模型本身仍严格 503/502）。纯函数 `buildCandidateBlock` / `sanitizeIndices`（越界 / 负数 / 去重，对应 ViMax `select_pairs_by_indices`）/ `stripFences`，测试 `DramaRenderServiceTest` 5/5。新增 `POST /me/drama/render/portraits`；`/me/drama/render/frame` 请求体加 `ref_pool`/`frame_desc`/`consistency`、响应加 `consistency`（openapi summary 同步）。
