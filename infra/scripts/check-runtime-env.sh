...
normalize_services() {
  ...
  for item in $raw; do
    case "$item" in
      ...
      "") ;;
      *) err "unknown service '$item' (expected server|web-music|web-drama|web-celebrity|web-aiavatar|admin|sau-service|all)" ;;
    esac
  done
...
needs_server_env() {
  has_service server || has_service admin || has_service web-music || has_service web-drama || has_service web-celebrity || has_service web-aiavatar || has_service sau-service
}

needs_release_manifest() {
  has_service admin || has_service web-music || has_service web-drama || has_service web-celebrity || has_service web-aiavatar
}
