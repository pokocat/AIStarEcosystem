...
server : AiModelPurpose +SELLING_POINTS / +VARIABLE_EXTRACT（SCRIPT_DRAFT 复用）
       ...
       : 新 MaterialAiService —— 文本三件薄流水线：resolve+fill → invokeChat → 解析/校验/
         自修复重试 1 次 → 占位兜底；脚本校验 blocks 3-8、变量过滤幻觉（原值须在脚本里出现）
...
web-celebrity:
       ...
       : DraftingHub AIPicker.run 接 ai-draft（失败回退本地 aiCandidates 占位池）
       : DeriveVariablesPanel 挂载时拉 extractScriptVariables（即时正则占位，AI 回来非空则升级）
...
test   : MaterialAiE2ETest（@MockBean AiModelInvocationService）—— 正常 JSON / 脏输出自修复 /
         无 provider 兜底 / 卖点 join / 变量过滤幻觉，6 测全绿
```

**注意事项**：

- **永远可降级**：provider 未配 / 调用失败 / JSON 解析失败 → 回退占位（脚本内置池 / 卖点 stub / 变量正则），
  HTTP 仍 200，前端无感。上线前需在 `/platform/ai-models` 配带 `SCRIPT_DRAFT/SELLING_POINTS/VARIABLE_EXTRACT`
  purpose 的 provider + 真 apiKey（模型 id 用「获取模型列表」选真实 id）。
...
- **未做**：脚本起稿计费（CelebrityActionPricingService 预留 action 未接 hold/commit）、违禁词 server lint 端点
  （前端纯规则已够用）、Langfuse 埋点、视频生成引擎 / RAG。
