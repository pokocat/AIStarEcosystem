...
- `ipstudio` AI IP 工作台（v0.151，真源 [`docs/ip-studio-plan.md`](docs/ip-studio-plan.md)）：新桌面子应用 `apps/web-ipstudio`（3015，React Flow 画布），**不新增产品码**，共用 aiavatar 开通、接口全挂 `/api/v1/ip-studio/**`（被 `ProductRouteTable` 的 `/api/v1/**` 兜底）。画布文档 `doc` 是**客户端拥有**、服务端整存整取不改内容；运行结果只落 `ip_run`（`GET project` 的 `runs` 按 nodeId 给最新一次、`runsById` 另收被 `selectedRunId` 选中的旧运行），禁止把运行结果写回 doc。生成链全部复用 dap 域（`DapMultimodalClient` / `DapImageInput` / `FileStorageService` / `PromptService` / `CreditService`），新 prompt key `dap.ip_identity`（带图 chat 抽人物特征卡，`chatJsonWithImages`）与 `dap.ip_look_image`（**v0.153 起形象卡只有一个自由 `prompt`**，整段塞进模板的 `{{outfit}}` 占位符 —— 不改模板名，运营存过自定义模板的实例才不会渲染不出内容；老画布五字段读取回落，零迁移），单价 `dap.ip-identity`=2 / `dap.ip-image`=8 后台可配。计费照 `DramaReferenceAssetService` 范式：preflight（引擎 / 提示词）在 hold 之前、整批一次 hold、每张成功 commit 后才写候选、首张失败释放剩余、`catch RuntimeException`；`IpRunReaper` 回收 15 分钟无心跳的运行并释放冻结。**worker 派发必须挂在 `afterCommit`**（`@Transactional` 里直接派发会让 worker 在提交前查不到行、任务永远 `queued`，真联调踩过）。一致性五层锁定：特征卡文本固定 / 主图作 i2i 锚（参考顺序 master → source → reference，超 `aep.ipstudio.max-ref-images`=4 按此砍尾并如实回报 `over_max_refs`）/ 风格文本原样拼接 / 服务端模板 + 一致性从句 / 多候选择优。发布零积分：建 `DapAvatar(path=ai, status=finalized)` + `DapLook(source=design)`，重复发布 409 `IP_PROJECT_ALREADY_PUBLISHED`。新表 `ip_project` / `ip_run` 走 **V27** SQL 迁移。
