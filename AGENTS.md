...
| 版本 | 日期 | 主题 |
|---|---|---|
| **v0.85** | 2026-06-18 | 短剧视觉一致性中间件（调研 [ViMax](https://github.com/HKUDS/ViMax) 后**移植其「参考注入」思路、不引入其依赖**：提示词进 `PromptService`、选择走既有 Java chat）。①**角色定妆三视图**：`POST /me/drama/render/portraits`（`drama.character_portrait` 提示词，正/侧/背三图落 CDN，单价 `drama.credit.portrait` 默认 6）→ 落 `CharacterDef.portraits`，角色卡「AI 生成三视图」入口。②**出图前一致性选择**（`renderFrame` 可降级旁路）：前端传 `ref_pool`（角色定妆图+场景参考图+本场上一镜首帧）+`frame_desc`，后端用 `drama.ref_select`（文本 chat，复用 `DRAMA_SCRIPT_DRAFT` 端点，**不需视觉模型**）挑 ≤6 张参考 + 改写「参考使用说明」拼进出图 prompt + 选中图作 `ref_images` 注入；响应回 `consistency:{used,clause,selected,reason}`，工厂抽屉展示「本帧一致性参考」。async frame-job 经 `DramaFrameJobWorker` 回放 body 同样生效（零改动）。③**场景参考图**：cast 阶段「AI 生成场景参考图」（空镜建立镜头）存进**素材场景库**（`Material.url`，cat=场景），工厂 `@素材参考` 选用即进池。§8.0：未配置/调用/解析失败→降级回原始出图（真实产物 `used=false`，不伪造不阻断不额外扣费）；`sanitizeIndices` 越界/负数/去重保护（对应 ViMax `select_pairs_by_indices`）。门禁：server compile + `DramaRenderServiceTest` 5/5 + typecheck:all + typecheck:admin + check:api-contract + web-drama build 全绿 |
