...
| 版本 | 日期 | 主题 |
|---|---|---|
| **v0.144** | 2026-08-31 | 短视频线遗留缺陷收口（无表变更）：① **出片计费不可绕过** —— 总装前核验每镜视频确实来自本账号 `MaterialVideoJob` 产物（本草稿 `script_id` 优先、退到账号级；按资产路径比对以容忍签名重签），不命中 400 `DRAMA_SHORT_CLIP_UNVERIFIED`，核验在配额与 ffmpeg 之前；此前伪造 `flow=done + videoUrl` 可跳过逐镜扣费直接成片（v0.76/v0.133 起的既有洞）。② **开拍费幂等** —— `POST /me/drama/shorts` 接 `clientRequestId`，owner+key 在 2h 窗口查重（键落 payloadJson，无唯一索引 → 只挡顺序重试），三个入口都带键。③ 分镜表加逐镜出场人物 chip（`FormShot.castNames`，与短剧线 `cast` id 分开），「加一镜」继承上一镜人物与场景。④ 人物/场景可增删并联动清理各镜引用。⑤ 拆解加单账号限频（5 分钟 10 次，429 `DRAMA_PROMPT_RATE_LIMITED`）。⑥ **部署时发现 Flyway 编号漂移**（生产 V22 / 仓库只到 V20，缺 V2–V13、V21、V22）—— 查清前禁止新增迁移，下一个编号按 V23，详见 TODO.md P0。 |
