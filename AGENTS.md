...
| 版本 | 日期 | 主题 |
|---|---|---|
| **v0.172** | 2026-09-08 | **签名 URL 一直是 403 —— V4 签完再改 host**（用户提出「是不是参考图 URL 读不了」，顺着查到根因）。证据链：火山的响应 `usage` 里**始终没有 `input_images`**（文档示例带参考图时有），说明它确实没用那张图;而对象本身**不带参数取得到（200，1.5MB）、一带上签名参数就 403**。根因：OSS 客户端用**内网 endpoint** 构造 + `SignVersion.V4`，签完再把 `-internal.aliyuncs.com` 字符串替换成公网域名 —— **V4 的签名覆盖 host**，改了 host 签名就对不上，OSS 校验失败回 403。隐蔽在于:桶是公开可读的，不带参数反而拿得到,带上一个对不上的签名 OSS 才去校验并拒掉 —— 于是「URL 看着没问题、点开也能下」的直觉全是错的。改为**用公网 endpoint 单独建一个只做预签名的客户端**（上传仍走内网，省流量费也更快），签名与 host 从一开始就一致，不再事后改写。⚠️ **同时查出一个安全问题**（未修，需产品决策）：`aiartist` 桶当前**公开可读** —— 不带签名直接 GET 任意对象返回 200。这让 §4.7 那整套签名 TTL 形同虚设:任何泄露出去的 key 都是永久可读。要么把桶改私有（那么这次的签名修复就是前提条件），要么承认这些资产是公开的、别再假装签名有保护作用。已记 TODO。门禁：server **986/986** + `check:api-contract` 全绿。 |
