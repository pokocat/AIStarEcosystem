...
```
server : DevFakeAiSeeder（@ConditionalOnProperty aep.dev-fake-llm.enabled，默认关闭，需显式 AEP_DEV_FAKE_LLM_ENABLED=true）：接入 fake 端点 +
...
- **能力切真实**：默认 `AEP_AIAVATAR_APP_MODE=live`，真实 provider 未配置时任务失败显性报错，不回退 mock；离线演示显式用 `AEP_AIAVATAR_APP_MODE=mock` 或 `AEP_AIAVATAR_PROVIDERS_<CAP>=mock`。自部署能力用 `AEP_AIAVATAR_PROVIDERS_<CAP>=selfhost` + `AEP_AIAVATAR_SELFHOST_BASE_URLS_<CAP>=...`；
...
web-aiavatar（重建）:
  ...
  · .env.development 默认 NEXT_PUBLIC_USE_MOCK=0（dev 真实后端）；离线演示显式 NEXT_PUBLIC_USE_MOCK=1。
