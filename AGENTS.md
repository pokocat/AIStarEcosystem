...
- 视频生成（v0.130）：聚算 JusuanHub `minimax-h3` 走独立媒体 Job 协议（`/media/generations` → `/jobs/{id}` → 受保护 `/assets/{id}/content` 镜像 OSS），不能按 GENERIC `/videos/generations` 调。端点 `billingMode=PER_SECOND` + 候选 `creditCostOverride=40` 表示 40 积分/秒，实际 hold=费率×时长；存量端点仍按次。当前候选只开放 t2v，首/尾帧资产上传未接通前必须保持 `maxRefImages=0`、`supportsFirstLastFrame=false`。
