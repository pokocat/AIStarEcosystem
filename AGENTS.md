...
### v0.34（2026-05-27）— 阿里云部署架构重构（infra/ + Flyway + seeder gate + 密钥 fail-fast）

把当前"单 ECS + localhost MariaDB + 本地盘 cdn-mock + 手工 rsync"的部署模式，重构成
**可版本化、可一键复制、可平迁到阿里云 RDS + OSS 的目标架构**。范围仅包含 Phase 0+1+2
（基础设施版本化、生产硬伤修复、RDS/OSS 配置就绪 + 迁移脚本）；Phase 3（全栈容器化 +
CI/CD）、Phase 4（用户上传素材 OSS 化）、Phase 5（多实例 Redis + ShedLock）待后续 v0.35+。

**A. 基础设施版本化（Phase 0；纯新增，零线上影响）**

新增 `infra/` 目录，把过去散落在 `/etc/nginx/`、`/etc/systemd/`、`/etc/aistareco/`
下的部署资产**全部归一到 git**：

```
infra/
├── README.md                    ← 入口 + 拓扑图 + 一次性拉起 SOP
├── MIGRATION.md                 ← 从单机 MariaDB+本地盘 迁到 RDS+OSS 的 SOP
├── env/                         ← 7 份 *.env.example（server / sau-service / 5 个 web app）
├── nginx/                       ← ai.conf.example（当前 IP）+ ai.aistar.com.conf.example（HTTPS 多子域目标）
├── systemd/                     ← 7 个 *.service.example（server / web / admin / 3 个新 web / sau-service）
├── rds/                         ← 00_create_database.sql + 01_create_app_user.sql + README
├── oss/                         ← ram-policy.json + cors-config.json + lifecycle.xml + README
└── scripts/                     ← deploy.sh / rollback.sh / verify.sh / migrate-db.sh / migrate-cdn.sh
```
...
miniprogram : 新增 config/env.example.js（apiBaseUrl + useMock 多环境配置）；
              app.js 改用 try { require("./config/env.js") } catch fallback，
              避免硬编 https://api.aistareco.local/api
.gitignore  : +apps/miniprogram/config/env.js  +infra/env/*.env （保留 *.example）
DEPLOYMENT.md : 顶部加 v0.34 节 + 指向 infra/README.md 的入口指示牌
```

**C. RDS / OSS 真切就绪（Phase 2）**
...
- `infra/scripts/migrate-db.sh` 一次性从 source ECS mysqldump 直推 RDS + 两边 count 对比
- `infra/scripts/migrate-cdn.sh` 用 ossutil sync ./cdn-mock + ./static/videos → OSS 前缀
- `infra/MIGRATION.md` 两种灰度路径（原地切 vs 新 ECS 全新部署）+ 常见故障排查

**注意事项**：

- **Flyway baseline 当前是空文件**（仅占位）。Flyway 启动时看到现存 schema 但无 schema_history
  会自动 baseline 到 V1，**不执行**本文件 → 兼容现有线上库。真正 schema 改动从 V2__xxx.sql 起。
  推荐在切 RDS 时把当前线上 schema mysqldump 出来作为 V1 完整 baseline，然后切 ddl-auto=validate。
...
- **deploy.sh 仍走 rsync + ssh**（与现有手工流程兼容）；容器化 + GitHub Actions 是 Phase 3。
