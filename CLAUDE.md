...
> - ✅ **Phase 4b**：`apps/web/src/components/celebrity-zone/` 33 个组件迁入 `apps/web-celebrity/src/components/celebrity-zone/`；工作台路由采用 route group `(workspace)`（不出现在 URL），生产级顶层路径就位：`/dashboard`（今日总览）、`/market`（明星市场）、`/cast`（我的明星）、`/projects` + `/projects/[projectId]`、`/products`（商品库）、`/library`（视频中心）、`/data`（数据中心），明星详情独立段 `/star/[starId]` + `/star/[starId]/apply` + `/star/[starId]/generate`；`(workspace)/layout.tsx` 提供独立 sidebar + topbar shell（`CelebrityShellProvider`，复用 `useAuth` 的 logout / wallet）；鉴权 wall 由 `app/providers.tsx` 的 `AuthProvider`（publicPrefixes `["/","/login","/activate"]`，`"/"` exact-match）承担；landing CTA 通过 `ProductLanding.postLoginPath="/dashboard"` 让登录后落到工作台。所有组件 / mocks 中 hard-coded 的 `/producer/celebrity-zone/...` 与旧 `/console/...` 链接已统一替换为新顶层路径。旧 `/console[?tab=xxx]` 由 `src/proxy.ts` 308 重定向到对应新路径（与 drama 同节奏，无残留旧书签后可删）。`pnpm typecheck:all` 七个 workspace 全绿；`pnpm dev:celebrity` 后 `/`、`/dashboard`、`/market`、`/star/star-liu-tao` 均 HTTP 200。
> - ✅ **Music 路由生产级化**（同 Phase 4b 风格）：`apps/web-music/src/app/console/*` → `(workspace)/*`，去 `/console` 前缀；overview 路径 `/console` → `/dashboard`；`producer-shell-context.tsx` 的 `navigate(page)` 实现改为 id→href 映射（`overview → /dashboard`，其余 `/${id}`）；layout legacy `?tab=` redirect 块删除，统一由 `src/proxy.ts` 308 重定向兼容旧链接。`postLoginPath` / `defaultPostLoginPath` 同步改为 `/dashboard`。三个子 app（music / drama / celebrity）现已全部统一为「`(workspace)` route group + 顶层语义化路径 + 旧 `/console` 由 `proxy.ts` 兼容」的形态。
>
> **尚未做**：
