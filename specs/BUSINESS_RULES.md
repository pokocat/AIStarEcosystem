...
- 提现（v0.45）：`CreditService.withdraw` 写 WITHDRAW LedgerEntry（amount<0），**仅扣 recharge 桶**
  （真实充值 / 业务收益可提现；license / gift 桶不可提现）；可提现余额不足 → HTTP 402。
```

参考：`apps/server/src/main/java/com/aistareco/aep/service/CreditService.java`。

> **drama 分发发布任务（v0.45）**：`/api/distribution/jobs/**`（`DramaPublishJob`）当前是
> 服务端进度模拟（读时推进 queued → uploading → transcoding → publishing → live，约 15s），
> **不做真实平台投递**。要真发布需接 sau-service（同 celebrity 的社媒账号绑定 + PublishJob 体系）。
