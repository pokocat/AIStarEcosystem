...
| `credits.voice.tts` | TTS 文字转语音消耗积分 | 1 | 积分/次 | |
| `credits.image.generate` | AI 图片生成消耗积分 | 2 | 积分/张 | |
| `credits.lyrics.generate` | AI 歌词生成消耗积分 | 1 | 积分/次 | |
...
| `credits.pose.unlock.rare` | 解锁稀有姿态消耗积分 | 10 | |
| `credits.pose.unlock.epic` | 解锁史诗姿态消耗积分 | 30 | |
| `credits.pose.unlock.legendary` | 解锁传说姿态消耗积分 | 80 | |

#### C. 套餐额度限制 — 可配置限额

| 配置项 Key | 功能描述 | free | pro | enterprise |
| --- | --- | --- | --- | --- |
| `quota.singer_slot.free` | free 套餐最大 AI 歌手创建数 | 3 | — | — |
| `quota.singer_slot.pro` | pro 套餐最大 AI 歌手创建数 | — | 20 | — |
| `quota.singer_slot.enterprise` | enterprise 套餐最大 AI 歌手数 | — | — | -1（无限） |
| `quota.nft_mint.monthly.pro` | pro 套餐每月 NFT 铸造次数 | 0 | 10 | -1（无限） |
| `quota.nft_mint.monthly.enterprise` | enterprise 套餐每月 NFT 铸造次数 | — | — | -1（无限） |
| `quota.distribution_channel.free` | free 套餐可用发行渠道等级 | domestic | — | — |
| `quota.distribution_channel.pro` | pro 套餐可用发行渠道等级 | — | all | — |
| `quota.distribution_channel.enterprise` | enterprise 套餐可用发行渠道等级 | — | — | priority |
| `quota.chart_vote.daily` | 每日榜单投票次数上限（全套餐通用） | 3 | 3 | 3 | |
| `quota.music_generate.daily.free` | free 套餐每日音乐生成次数上限 | 5 | — | — |
| `quota.music_generate.daily.pro` | pro 套餐每日音乐生成次数上限 | — | 50 | — |
| `quota.track_publish.monthly.free` | free 套餐每月可发行曲目数 | 3 | — | — |
| `quota.track_publish.monthly.pro` | pro 套餐每月可发行曲目数 | — | 30 | — |
| `quota.api_call.per_minute` | API 调用频率限制（次/分钟） | 30 | 120 | 600 |
...
| `incentive.first_singer.gift_credits` | 首次创建歌手奖励积分 | 50 | |
| `incentive.first_publish.gift_credits` | 首次发行作品奖励积分 | 20 | |
| `incentive.daily_login.gift_credits` | 每日登录签到奖励积分 | 5 | |
| `incentive.invite.inviter_credits` | 邀请好友注册，邀请人奖励积分 | 200 | |
| `incentive.invite.invitee_credits` | 受邀新用户注册奖励积分 | 50 | |
| `incentive.plan_monthly_credit.free` | free 套餐每月赠送积分（月重置） | 50 | |
| `incentive.plan_monthly_credit.pro` | pro 套餐每月赠送积分（月重置） | 500 | |
| `incentive.plan_monthly_credit.enterprise` | enterprise 套餐每月赠送积分 | -1（无限） | |
...
| `revenue.signing.platform_ratio` | 签约费平台服务费比例 | 0.20 | 小数 | 与 seller_ratio 之和=1 |
| `revenue.operation.operator_ratio` | 运营收益买家（运营方）分成 | 0.70 | 小数 | |
| `revenue.operation.creator_ratio` | 运营收益原创者分成 | 0.30 | 小数 | |
| `revenue.nft_royalty.default_bps` | NFT 版税默认建议值（basis points） | 1000 | bps (=10%) | 铸造时可覆盖 |
| `revenue.nft_royalty.min_bps` | NFT 版税最小值 | 0 | bps | |
| `revenue.nft_royalty.max_bps` | NFT 版税最大值 | 10000 | bps (=100%) | |
| `revenue.platform_fee_rate` | 充值/订单平台手续费率 | 0.02 | 小数 | |

#### G. 价格配置 — 套餐/积分包定价

| 配置项 Key | 功能描述 | 默认值 | 备注 |
| --- | --- | --- | --- |
| `pricing.plan.pro.monthly_usd` | pro 套餐月付价格（美元） | 29.99 | |
| `pricing.plan.pro.yearly_usd` | pro 套餐年付价格（美元） | 299.99 | |
| `pricing.plan.enterprise.monthly_usd` | enterprise 套餐月付价格 | 99.99 | |
| `pricing.addon.credit_100.usd` | 100 积分加油包价格（美元） | 1.99 | |
| `pricing.addon.credit_500.usd` | 500 积分加油包价格（美元） | 8.99 | |
| `pricing.addon.credit_2000.usd` | 2000 积分加油包价格（美元） | 29.99 | |
| `pricing.plan.pro.monthly_cny` | pro 套餐月付价格（人民币） | 199 | 国内定价 |
| `pricing.plan.pro.yearly_cny` | pro 套餐年付价格（人民币） | 1999 | |
...
| `ui.credits_warn_before_generate` | 生成任务前积分预览弹窗开关 | true | |
| `ui.upgrade_prompt.music_generate` | 音乐生成积分不足时升级引导文案 Key | `upgrade.music` | 指向 i18n key |
| `ui.upgrade_prompt.nft_mint_locked` | NFT 铸造被锁定时升级引导文案 Key | `upgrade.nft` | |
| `ui.upgrade_prompt.singer_quota` | 歌手名额满时升级引导文案 Key | `upgrade.singer_slot` | |
| `ui.onboarding_steps` | 新用户引导步骤配置（JSON 数组） | [...] | 可配置步骤顺序 |
| `ui.announcement_banner` | 全站顶部公告栏文案（空则不显示） | "" | 支持 zh/en |
| `ui.credits_topup_redirect` | 充值入口跳转地址 | `/billing/topup` | |
...
#### 后台管理接口（需 platform_operator 以上权限）
...
| 配置分组 | 可修改角色 | 是否需审批 |
| --- | --- | --- |
| `credits.*`（积分消耗） | platform_operator + | 建议双人审批 |
| `quota.*`（套餐限额） | platform_operator + | 建议双人审批 |
| `incentive.*`（赠送参数） | platform_operator + | 需审批 |
| `revenue.*`（收益分成） | platform_owner only | 必须审批 |
| `pricing.*`（价格配置） | platform_owner only | 必须审批 |
| `feature.*`（功能开关） | platform_operator + | 视开关影响范围决定 |
| `ui.*`（界面行为） | platform_operator + | 无需审批 |
