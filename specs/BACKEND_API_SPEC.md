...
**生成日期**: 2026-04-14  
**前端技术栈**: Next.js 14 + TypeScript  
**文档状态**: ✅ 已按 2026-04-14 需求会更新  

## 文档版本迭代记录

| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v1.1.0 | 2026-04-14 | 依据 2026-04-14 需求对齐会议，补充 AI艺人/IP 孵化、买断/授权合同、外部平台收益回流、激活码/配额/积分结算模型。 |
| v1.0.0 | 2026-04-08 | 仓库现有接口与数据模型基线版本。 |
...
### 1.21 合同类型 `ContractType`
```
buyout                 买断转让
exclusive_license      独家运营授权
non_exclusive_license  非独家授权
```

### 1.22 权利范围 `RightsScopeKey`
```
image           形象/肖像使用权
music_catalog   音乐资产包
distribution    发行运营权
ad_campaign     广告投放/商业合作
short_video     短视频内容使用
social_account  社交账号/粉丝资产
```

### 1.23 激活码状态 `ActivationCodeStatus`
```
created     已生成
imported    已入库
sold        已售出
...
### 1.24 激活码结算模式 `ActivationSettlementMode`
```
prepaid        发码即结算
on_activation  激活后结算
```

### 1.25 编辑能力等级 `EditorTier`
```
lite      轻编辑/哼唱版
assisted  外设辅助版
pro       专业音色库版
...
interface Singer {
  ...
  workspace_id: string;          // 关联租户/工作区，映射经纪公司主体
  ...
  template_type: string | null;  // official_ip / founder_ip / brand_ip / custom
  commercial_rights_status: 'self_owned' | 'licensed' | 'restricted';
...
| `workspace_id` | UUID | 是 | — | 对应经纪公司/工作区 |
| `template_type` | string | 否 | `null` | 模板化孵化来源 |
...
interface Track {
  ...
  editor_tier: EditorTier;         // 'lite' | 'assisted' | 'pro'
  source_provider: string | null;  // 生成引擎名称
  trim_in_sec: number | null;      // 轻编辑起始时间
  trim_out_sec: number | null;     // 轻编辑结束时间
...
interface TrackGenerationRequest {
  ...
  editor_tier: EditorTier;            // 编辑能力等级，默认 'lite'
  ...
  provider_preference: string | null; // 用户偏好引擎（可选）
  allow_external_editor: boolean;     // 是否允许跳转外部编辑器
}
```
...
interface TrackGenerationResponse {
  ...
  provider_name: string | null;       // 实际执行的引擎
  can_open_external_editor: boolean;  // 是否可跳转第三方编辑页
...
interface MarketplaceArtist {
  ...
  workspace_id: string;          // 挂牌所属工作区/经纪公司
  name: string;                  // 艺人名称
  avatar_url: string;            // 头像图片 URL
  style: string;                 // 音乐风格
  ...
  license_type: LicenseType;     // 兼容前端简单选择
  contract_types: ContractType[];// 支持的合同类型
  default_contract_type: ContractType;
  rights_scope_options: RightsScopeKey[]; // 可授权权利包
  default_duration_days: number | null;   // 默认授权时长，买断时为 null
  allow_buyout: boolean;         // 是否允许买断
  enable_auto_reply: boolean;    // 是否开启自动回复询价
  settlement_mode: 'external_account' | 'platform_managed'; // 收益默认回流方式
...
interface ArtistAnalytics {
  ...
  revenue_source_notes: string[]; // 收益来源说明，如“平台外结算”
  updated_at: string;
}
...
interface SigningContract {
  ...
  workspace_id: string;           // 合同归属工作区
  singer_id: string;              // 关联 Singer.id
  contract_type: ContractType;    // 合同类型
  rights_scope: RightsScopeKey[]; // 实际授权范围
  signing_price: number;          // 签约价格（元）
  duration_days: number | null;   // 授权时长，买断时为 null
  territory: string | null;       // 地域范围，如 CN / GLOBAL
  transfer_existing_tracks: boolean; // 是否转移历史曲目版权
  include_social_accounts: boolean;  // 是否包含社交账号/粉丝资产
  revenue_split_buyer: number | null;  // 授权模式下默认 70
  revenue_split_seller: number | null; // 授权模式下默认 30
  settlement_routing: 'external_account' | 'platform_managed';
...
```
1. 合同类型必须在买断、独家授权、非独家授权中明确选择
2. 形象权、音乐版权、发行运营权、广告投放权、社交账号资产默认不打包，需逐项勾选
3. 若为授权合同，默认收益分成比例可配置为您 70% / 原创者 30%
4. 若为买断合同，需明确历史曲目与后续收益是否一并转移
5. 若收益回流到外部平台账号，平台仅记录合同关系与回传数据，不替代第三方平台打款
6. 您需遵守平台社区规范、肖像权与版权合规要求，不得从事违法违规活动
```
...
interface ArtistListingRequest {
  ...
  license_type: LicenseType;   // 兼容前端简单挂牌选择
  contract_types: ContractType[];   // 支持的合同类型
  rights_scope_options: RightsScopeKey[]; // 可授权权利范围
  default_duration_days: number | null;   // 默认期限
...
interface PlatformAccount {
  ...
  account_name: string | null;    // 平台侧账号名称
  ...
  revenue_owner_type: 'user' | 'workspace'; // 收益归属主体
  settlement_enabled: boolean;    // 是否允许回流收益数据
...
interface DistributionJob {
  ...
  revenue_settlement_mode: 'external_account' | 'platform_managed';
  result_summary_url: string | null; // 外部渠道结果页
  submitted_at: string;
  completed_at: string | null;
}
...
interface PlatformResult {
  ...
  settlement_status: 'external' | 'managed' | 'unknown';
  error_message: string | null;
}
```
...
#### `ActivationCodeBatch` — 激活码批次
> 来源：2026-04-14 需求对齐会议（账号授权、配额、分销、对接艺人公司）

```typescript
interface ActivationCodeBatch {
  id: string;
  batch_no: string;                         // 批次号，唯一
  settlement_mode: ActivationSettlementMode;// 'prepaid' | 'on_activation'
  singer_slot_quota: number;                // 默认开通艺人名额
  initial_credit_amount: number;            // 初始赠送积分
  expires_at: string | null;
  external_batch_no: string | null;         // 外部系统批次号（如艺人公司）
  imported_from: string | null;             // 来源系统标识
  created_at: string;
}
```
...
#### `ActivationCode` — 单个激活码
> 来源：2026-04-14 需求对齐会议（激活码入库匹配、按发码或激活结算）

```typescript
interface ActivationCode {
  id: string;
  batch_id: string;                  // 关联 ActivationCodeBatch.id
  code_masked: string;               // 脱敏后的展示值
  status: ActivationCodeStatus;      // 'created' | 'imported' | 'sold' | 'activated' | ...
  workspace_id: string | null;       // 激活后关联的工作区
  sold_to_partner_id: string | null; // 售卖给哪个渠道/代理
  activated_by_user_id: string | null;
  activated_at: string | null;
  imported_at: string | null;        // 入库时间，用于“双边匹配”校验
  created_at: string;
}
```
...
#### `WorkspaceEntitlementSnapshot` — 工作区授权快照
> 来源：2026-04-14 需求对齐会议（一个账号默认 3 个艺人名额，激活码决定额度与积分）

```typescript
interface WorkspaceEntitlementSnapshot {
  workspace_id: string;
  plan_type: PlanType;
  singer_slot_total: number;
  singer_slot_used: number;
  editor_tier: EditorTier;
  available_credits: number;
  last_activation_code_id: string | null;
  updated_at: string;
}
```
...
POST   /api/marketplace/:id/sign     签约艺人（购买）
  body: {
    ...
    transfer_existing_tracks,
    include_social_accounts
  }

GET    /api/marketplace/contracts/:id 获取签约合同详情
PATCH  /api/marketplace/contracts/:id/terminate 终止合同
PATCH  /api/marketplace/contracts/:id/renew     续签/延期合同
...
GET    /api/activation-codes/:code/status  查询激活码状态（管理员/渠道）
GET    /api/workspace/entitlements         获取当前工作区授权快照

POST   /api/admin/activation-code-batches        创建激活码批次
POST   /api/admin/activation-code-batches/import 导入外部批次/码库存
GET    /api/admin/activation-code-batches/:id    查看批次详情
```
...
| 409 | `ACTIVATION_CODE_NOT_IMPORTED` | 激活码未入库匹配，暂不可用 |
...
*文档结束 — AI Star Eco 前后端数据模型对接规范 v1.1.0*  
*前端源码分析完成时间：2026-04-14*  
*如有字段变更请同步更新本文档*
