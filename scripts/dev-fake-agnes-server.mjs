#!/usr/bin/env node
// ============================================================
// 本地 fake Agnes 服务器（零依赖，OpenAI 兼容协议子集）
//   配套 apps/server 的 dap 领域（AGNES_BASE_URL=http://localhost:18181）
//   让「数字人资产平台」全链路在无外网 / 无真实 key 时端到端跑通：
...
// 用法：node scripts/dev-fake-agnes-server.mjs   # 端口 18181（FAKE_AGNES_PORT 覆盖）
// ============================================================
import http from "node:http";
import zlib from "node:zlib";

const PORT = Number(process.env.FAKE_AGNES_PORT || 18181);
...
server.listen(PORT, () => console.log("[fake-agnes] listening on http://localhost:" + PORT));
